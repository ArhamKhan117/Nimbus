"""Screen capture + cursor + DPI + resolution picking for Nimbus.

Produces Nimbus-ready images (exact target dimensions) with the metadata
needed for coordinate unscaling across the physical / logical / declared
coordinate spaces.

Responsibility boundary:
- THIS MODULE lives in Space A (physical pixels) and Space C (Nimbus's
...
Top-to-bottom order (so `python -m capture` works):
    ...
    4. Pure functions (pick_resolution, resize_for_model, unscale_model_coords)
...
@dataclass
class CaptureResult:
    """All the metadata needed to send a screenshot to Nimbus and later
    map Nimbus's returned coordinates back to the user's physical screen.
...
class LabeledCapture:
    """Extended capture result with a label string for the multi-screen context.

    Each captured screen gets a label like
    "screen 1 of 2 — cursor is on this screen (primary focus) (image dimensions: 1280x800 pixels)".

    Attributes:
        image: PIL Image, already resized to exact (target_width, target_height).
        label: human-readable label for Nimbus's context (includes pixel dims).
...
def _aspect_preserving_size(width: int, height: int) -> tuple[int, int]:
    """Scale (width, height) down by ONE uniform factor to fit the model bounds (T2-8).

    Used when no candidate resolution matches the monitor's aspect ratio. Because a single
    factor is applied to both axes, ``scale_x == scale_y`` and the image the model sees is
    geometrically faithful -- which is the whole point.

    Never upscales: the factor is capped at 1.0, so a small display is returned untouched
    exactly as the candidate path would.
    """
    factor = min(
        MAX_MODEL_LONG_EDGE / max(width, height),
        MAX_MODEL_SHORT_EDGE / min(width, height),
        1.0,
    )
    # At least 1px per axis: a degenerate monitor must not produce a zero dimension.
    return (max(1, round(width * factor)), max(1, round(height * factor)))


def pick_resolution(width: int, height: int) -> tuple[int, int]:
    """Pick a model-friendly capture size for a monitor.

    The goal is to avoid distortion when resizing a monitor's image
    down to a model-friendly resolution. Picking a resolution whose
    aspect ratio is closest to the source minimizes stretching and preserves
    X/Y coordinate accuracy.

    **T2-8 -- ultrawide correctness.** ``CANDIDATE_RESOLUTIONS`` spans aspect 1.333 to
    1.778, so an ultrawide has no acceptable entry and "closest" is still badly wrong: a
    3840x1080 (32:9) monitor picked 1920x1080 and was squashed **2x horizontally**, giving
    ``scale_x=2.0, scale_y=1.0``. Circular icons reached the model as ellipses. Measured
    cost: 4/6 ground-truth targets hit and 50px max error, versus 6/6 and 15px once the
    aspect was preserved (see ``config.MAX_MODEL_LONG_EDGE``).

    So when the best candidate's aspect drifts more than ``ASPECT_TOLERANCE``, fall back to
    a uniformly scaled size instead. Every 4:3 / 16:10 / 16:9 monitor is unaffected and
    keeps byte-identical behaviour -- only shapes the list cannot express change.

    ``scale_x != scale_y`` in the returned ``CaptureResult`` is the diagnostic for this
    class of bug; after this change it should never happen.
    ...
    Returns:
        (target_w, target_h). Usually from ``config.CANDIDATE_RESOLUTIONS``; a computed
        aspect-preserving size when no candidate fits.
    ...
    # Checked BEFORE the aspect test so a 5:4 panel (1280x1024) still returns native
    # rather than being routed through the fallback for a 6% aspect drift.
    if width <= best[0] and height <= best[1]:
        return width, height
    if abs(source_ratio - (best[0] / best[1])) / source_ratio > ASPECT_TOLERANCE:
        return _aspect_preserving_size(width, height)
    return best
...
def resize_for_model(
    ...
    Computes the scale factors needed to later unscale Nimbus's returned
    coordinates back to the source image's pixel space:
        physical_x = model_x * scale_x
        physical_y = model_y * scale_y
...
def unscale_model_coords(
    ...
    """Map Nimbus's (x, y) in declared resolution space -> physical pixels
    in virtual desktop coords.

    Clamps model_x / model_y to [0, target_w - 1] and [0, target_h - 1]
    BEFORE scaling. The model occasionally returns slightly out-of-bounds
    coordinates, so we clamp to keep the pointer on-screen.

    Args:
        model_x: x coordinate Nimbus returned, in declared resolution space.
        model_y: y coordinate Nimbus returned, in declared resolution space.
        scale_x: physical width / target_w (from resize_for_model).
        scale_y: physical height / target_h (from resize_for_model).
        monitor_left: monitor's x offset in virtual desktop coords.
        monitor_top: monitor's y offset in virtual desktop coords.
        target_w: the declared width we sent to Nimbus (for clamping).
        target_h: the declared height we sent to Nimbus (for clamping).

    Returns:
        (physical_x, physical_y) in virtual desktop pixel coordinates,
        ready for the overlay's physical->Qt-logical conversion.
    """
    clamped_x = max(0, min(model_x, target_w - 1))
    clamped_y = max(0, min(model_y, target_h - 1))
...
def capture_active_screen() -> CaptureResult:
    """Capture the monitor currently under the cursor, resize for Nimbus.
    ...
    Returns:
        ...
        to later unscale Nimbus's coordinates back to physical pixels.
    """
    ...
    resized, scale_x, scale_y = resize_for_model(raw_img, target_w, target_h)
...
def capture_all_screens() -> list[LabeledCapture]:
    """Capture all connected monitors, resize each for Nimbus, return sorted
...
if __name__ == "__main__":
    print("=" * 70)
    print("Nimbus -- capture.py manual verification")
    print("=" * 70)
    ...
    print("\nResolution picked for Nimbus (closest aspect-ratio match):")
    print(f"  {result.target_width} x {result.target_height}")

    print("\nScale factors (physical / target, for unscaling Nimbus coords):")
    print(f"  scale_x = {result.scale_x:.4f}")
    print(f"  scale_y = {result.scale_y:.4f}")

    print("\nResized image dimensions (what Nimbus will see):")
    ...
    demo_model = (640, 400)
    demo_physical = unscale_model_coords(
        model_x=demo_model[0], model_y=demo_model[1],
    ...
    print("\nDemo unscale:")
    print(
        f"  if Nimbus says 'click at {demo_model}' in "
