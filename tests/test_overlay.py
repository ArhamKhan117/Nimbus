...
screen_for_monitor, OverlayController._overlay_for_screen lookup.
...
All of the above are verified manually by running `py -3.13 -m overlay`.
"""
...
class TestPhysicalToLocalLogical:
    ...
    def test_200pct_dpi_primary_at_origin(self):
        ...
        Matches verified coordinate math."""
        from overlay import physical_to_local_logical
...
class TestOverlayControllerLifecycle:
    ...
    These methods protect the screenshot-integrity invariant -- if Nimbus
...
class TestOverlayVisualHelpers:
    ...
    def test_state_accent_mapping_is_centralized_and_semantic(self):
        """EXPECTATION CHANGED (SHELL_AND_CHAT.md §2.4 — retheme to Nimbus orange).

        POINTING was blue (59,130,246) and IDLE was blue (96,165,250). The overlay is now
        themed with the application's accent, so POINTING is orange and IDLE recedes to a
        muted grey instead of glowing blue at rest.

        The old values were not wrong; the brand changed. What matters is that the mapping is
        still *centralised* and still *semantic*, which is what this test is actually for.
        """
        from overlay import _OverlayState, _accent_rgb
        import theme

        assert _accent_rgb(_OverlayState.POINTING) == theme.parse_hex(theme.ACCENT)
        assert _accent_rgb(_OverlayState.IDLE) == theme.parse_hex(theme.TEXT_MUTED)
        # Unchanged on purpose: recording indicators are green essentially everywhere, and
        # overriding that learned signal for palette consistency would cost the user
        # certainty about whether the microphone is live.
        assert _accent_rgb(_OverlayState.LISTENING) == (34, 197, 94)
        assert _accent_rgb(_OverlayState.THINKING) == (245, 158, 11)

    def test_overlay_palette_comes_from_the_design_system(self):
        """Drift guard: the overlay, the shell and the HUD must not diverge.

        Sourcing from theme.py is what makes a future retheme one edit rather than three.
        """
        from overlay import _STATE_ACCENT_RGB
        import theme

        assert set(_STATE_ACCENT_RGB.values()) <= set(
            theme.OVERLAY_STATE_RGB.values())
...
class TestWaveformBarHeight:
    """Tests for _waveform_bar_height — verifies the waveform bar math.
...
class TestSpinnerWidgetMath:
    """Tests for SpinnerWidget math helpers — verifies the rotation math."""
...
# --- Teaching-annotation coordinate transform -------------------------------
