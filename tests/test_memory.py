...
class TestMemoryStoreConstruction:
    ...
    def test_init_creates_memory_dir_lazily_on_first_record(self, tmp_path):
        ...
        store.record(
            ...
            model_response="Click View tab, Freeze Panes.",
...
class TestRecall:
    ...
    def test_recall_returns_full_markdown_when_smaller_than_max_chars(self, tmp_path):
        ...
        store.record(
            ...
            model_response="Click View -> Freeze Panes.",
            pointer_targets=[(1245, 82)],
        )
        recalled = store.recall("EXCEL.EXE")
        assert "# EXCEL.EXE — Nimbus Memory" in recalled
    ...
    def test_recall_returns_tail_when_larger_than_max_chars(self, tmp_path):
        ...
        big_content = "# BigApp — Nimbus Memory\n\n" + ("# filler line\n" * 1000)
...
class TestRecord:
    ...
    def test_record_appends_second_interaction_and_updates_index(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("memory.datetime", _FakeDatetime)
        ...
        assert md.count("## 2026-08-12") == 2

        apps = store.list_known_apps()
        assert apps[0]["interaction_count"] == 2
        assert apps[0]["first_seen"] == "2026-08-12 14:30"
        assert apps[0]["last_seen"] == "2026-08-12 15:45"
    ...
    def test_record_escapes_triple_backticks_in_user_question_and_response(self, tmp_path):
        ...
        Prevents Nimbus's ```python code fences from corrupting the markdown
        block shape that memory-file parsers rely on.
        """
        store = _make_store(tmp_path)
        store.record(
            app_name="vscode.exe",
            ...
            model_response="The ```python fence starts a code block.",
            pointer_targets=[],
        )
    ...
    def test_record_handles_empty_pointer_targets_as_text_only_marker(self, tmp_path):
        ...
        'Pointed at:' line, even when Nimbus returned text-only.
        """
        store = _make_store(tmp_path)
        store.record(
            app_name="chrome.exe",
            ...
            model_response="It's about search.",
            pointer_targets=[],
        )
    ...
    def test_record_uses_same_timestamp_for_markdown_and_sqlite(self, tmp_path, monkeypatch):
        ...
        class _FakeDatetime:
            @classmethod
            def now(cls):
                return datetime(2026, 8, 12, 15, 30)
        ...
        md = (tmp_path / "memory" / "excel.exe.md").read_text(encoding="utf-8")
        assert "First seen: 2026-08-12 15:30" in md
        assert "## 2026-08-12 15:30" in md

        apps = store.list_known_apps()
        assert apps[0]["first_seen"] == "2026-08-12 15:30"
        assert apps[0]["last_seen"] == "2026-08-12 15:30"


# --- TestListKnownApps -------------------------------------------------------

class TestListKnownApps:
    """list_known_apps() returns a sorted list of dicts."""
    ...
    def test_list_known_apps_sorted_by_last_seen_desc_with_correct_dict_shape(
        ...
        times = [
            datetime(2026, 8, 12, 10, 0),  # a.exe first
            datetime(2026, 8, 12, 11, 0),  # b.exe second
            datetime(2026, 8, 12, 12, 0),  # c.exe third
        ]
        idx = [0]
        ...
        # Dict shape is stable — callers rely on these keys.
        for app in apps:
            assert set(app.keys()) == {
                "app_name",
                "first_seen",
...
class TestRecallBounds:
    """T0-6: recall()'s docstring claimed a 3000-char default while
    config.MEMORY_RECALL_MAX_CHARS was 1500.

    The docstring is corrected, and these tests pin the two properties that were
    documented but never actually guarded: the cap is honoured, and the TAIL is
    returned (recent context wins) rather than the head.
    """

    def test_recall_respects_configured_max_chars(self, tmp_path):
        from memory import MemoryStore
        store = MemoryStore(
            memory_dir=tmp_path / "memory", index_db_path=tmp_path / "index.db"
        )
        store.record(
            app_name="BIG.EXE",
            window_title="w",
            user_question="q",
            model_response="x" * 5000,
            pointer_targets=[],
        )
        assert len(store.recall("BIG.EXE", max_chars=1500)) <= 1500

    def test_recall_returns_the_tail_not_the_head(self, tmp_path):
        """The documented contract is a tail read so the most recent
        interactions survive truncation."""
        from memory import MemoryStore
        store = MemoryStore(
            memory_dir=tmp_path / "memory", index_db_path=tmp_path / "index.db"
        )
        for i in range(40):
            store.record(
                ...
                model_response="padding " * 40,
                pointer_targets=[],
            )
        recalled = store.recall("APP.EXE", max_chars=600)
        assert len(recalled) <= 600
        assert "question number 39" in recalled, "newest interaction must survive"
        assert "question number 0" not in recalled, "oldest should be truncated away"

    def test_recall_default_matches_config_constant(self, tmp_path):
        """Drift guard: the default parameter must track config, so the
        docstring/constant mismatch cannot silently reappear."""
        import inspect
        import config
        from memory import MemoryStore
        default = inspect.signature(MemoryStore.recall).parameters["max_chars"].default
        assert default == config.MEMORY_RECALL_MAX_CHARS

    def test_recall_of_unknown_app_is_empty(self, tmp_path):
        from memory import MemoryStore
        store = MemoryStore(
            memory_dir=tmp_path / "memory", index_db_path=tmp_path / "index.db"
        )
        assert store.recall("NEVER_SEEN.EXE") == ""

    def test_non_positive_max_chars_fails_closed(self, tmp_path):
        """Python's text[-0:] returns the WHOLE string, so a zero or negative
        cap must be rejected explicitly rather than leaking the entire file."""
        from memory import MemoryStore
        store = MemoryStore(
            memory_dir=tmp_path / "memory", index_db_path=tmp_path / "index.db"
        )
        store.record(
            ...
            window_title="w",
            user_question="q",
            model_response="secret content",
            pointer_targets=[],
        )
        assert store.recall("APP.EXE", max_chars=0) == ""
        assert store.recall("APP.EXE", max_chars=-10) == ""
