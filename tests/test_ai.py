...
class TestParsePointTag:
    ...
    def test_malformed_tag_returns_no_coordinate(self):
        # T0-3: this previously asserted the malformed tag SURVIVED in
        # spoken_text. That leaked raw coordinates to TTS, violating the
        # never-speak-coordinates invariant — the user heard "open bracket
        # POINT colon garbage". The tag is now fail-closed stripped, and the
        # raw text is preserved on `malformed_tags` for the debug log so the
        # failure stays diagnosable.
        from ai import parse_point_tag
        result = parse_point_tag("broken tag [POINT:garbage]")
        assert result.coordinate is None
        assert result.spoken_text == "broken tag"
        assert "[POINT" not in result.spoken_text
        assert result.malformed_tags == ("[POINT:garbage]",)
...
# --- T0-1: Anthropic slug adaptation -----------------------------------------

class TestAnthropicModelForEndpoint:
    """T0-1: pure slug adaptation. No SDK, no network.

    The bug this guards: AnthropicClient stored model_id verbatim while the two
    reachable endpoints expect different formats, so BOTH key types failed.
    Native api.anthropic.com wants a bare dash-versioned id; OpenRouter wants a
    namespaced dot-versioned slug."""

    def test_native_strips_namespace_and_keeps_dashes(self):
        from ai import _anthropic_model_for_endpoint
        assert _anthropic_model_for_endpoint(
            "anthropic/claude-sonnet-4-6", None
        ) == "claude-sonnet-4-6"

    def test_openrouter_keeps_namespace_and_converts_to_dots(self):
        from ai import _anthropic_model_for_endpoint
        assert _anthropic_model_for_endpoint(
            "anthropic/claude-sonnet-4-6", "https://openrouter.ai/api"
        ) == "anthropic/claude-sonnet-4.6"

    def test_bare_native_id_unchanged(self):
        from ai import _anthropic_model_for_endpoint
        assert _anthropic_model_for_endpoint(
            "claude-sonnet-4-6", None
        ) == "claude-sonnet-4-6"

    def test_opus_and_haiku_convert(self):
        from ai import _anthropic_model_for_endpoint
        assert _anthropic_model_for_endpoint(
            "anthropic/claude-opus-4-6", "https://openrouter.ai/api"
        ) == "anthropic/claude-opus-4.6"
        assert _anthropic_model_for_endpoint(
            "anthropic/claude-haiku-4-5", "https://openrouter.ai/api"
        ) == "anthropic/claude-haiku-4.5"

    def test_only_trailing_version_pair_is_converted(self):
        """The dash→dot rewrite must be anchored to the version suffix so it
        cannot mangle dashes inside the model family name."""
        from ai import _anthropic_model_for_endpoint
        assert _anthropic_model_for_endpoint(
            "anthropic/claude-3-5-sonnet-20241022", "https://openrouter.ai/api"
        ) == "anthropic/claude-3-5-sonnet-20241022"

    def test_non_openrouter_custom_base_url_uses_native_form(self):
        """A self-hosted proxy is not OpenRouter, so it gets the native form."""
        from ai import _anthropic_model_for_endpoint
        assert _anthropic_model_for_endpoint(
            "anthropic/claude-sonnet-4-6", "https://proxy.internal/v1"
        ) == "claude-sonnet-4-6"

    def test_default_config_model_is_not_the_scrubbed_placeholder(self):
        """T0-1 drift guard: the placeholder must never return."""
        import config
        assert "model-sonnet" not in config.DEFAULT_ANTHROPIC_MODEL
        assert config.DEFAULT_ANTHROPIC_MODEL.startswith("claude-")


# --- T0-3: signed coordinates, unanchored tags, fail-closed stripping ---------

class TestParsePointTagSignedCoordinates:
    """T0-3 mode 1: the regex used \\d+, so a negative coordinate failed the
    whole match — pointing died AND the raw tag leaked into spoken_text.

    Negative coordinates are real: the model emits them when a target sits at
    or just past a screenshot edge."""

    def test_negative_x_parses(self):
        from ai import parse_point_tag
        result = parse_point_tag("just off the edge. [POINT:-3,50:save]")
        assert result.coordinate == (-3, 50)
        assert result.element_label == "save"
        assert result.spoken_text == "just off the edge."

    def test_negative_y_parses(self):
        from ai import parse_point_tag
        result = parse_point_tag("up above. [POINT:400,-12:tab]")
        assert result.coordinate == (400, -12)

    def test_both_negative_parse(self):
        from ai import parse_point_tag
        result = parse_point_tag("way off. [POINT:-8,-16:corner]")
        assert result.coordinate == (-8, -16)

    def test_negative_coordinate_is_not_clamped_here(self):
        """Space C is unclamped by contract — capture.unscale_model_coords owns
        clamping. Clamping here would break that boundary."""
        from ai import parse_point_tag
        assert parse_point_tag("x. [POINT:-999,-999:e]").coordinate == (-999, -999)

    def test_negative_coordinate_clamps_downstream(self):
        """The companion half of the contract: a negative Space-C coordinate
        must survive parsing and then be clamped by unscale_model_coords."""
        from ai import parse_point_tag
        from capture import unscale_model_coords

        coord = parse_point_tag("x. [POINT:-5,-5:e]").coordinate
        phys = unscale_model_coords(
            model_x=coord[0], model_y=coord[1], scale_x=2.0, scale_y=2.0,
            monitor_left=100, monitor_top=200, target_w=1280, target_h=800,
        )
        assert phys == (100, 200), "negative coords must clamp to the monitor origin"


class TestParsePointTagUnanchored:
    """T0-3 mode 2: the regex ended with \\]\\s*$, so any trailing prose after
    the tag meant no match, no strip, and the tag was spoken aloud."""
    ...
    def test_trailing_prose_after_tag_is_stripped(self):
        ...
        assert result.element_label == "save"
        assert "[POINT" not in result.spoken_text
        assert "hope that helps!" in result.spoken_text

    def test_multiple_tags_last_one_wins(self):
        """The prompt appends the tag after the answer, so the final tag is the
        operative one. All of them must still be stripped."""
        from ai import parse_point_tag
        result = parse_point_tag("a [POINT:1,2:x] b [POINT:3,4:y] c")
        assert result.coordinate == (3, 4)
        assert result.element_label == "y"
        assert "[POINT" not in result.spoken_text

    def test_lowercase_tag_parses_and_strips(self):
        """The system prompt asks for all-lowercase prose, so the model may emit
        a lowercase tag. annotations.py already tolerates this; ai.py now does."""
        from ai import parse_point_tag
        result = parse_point_tag("look. [point:12,34:btn]")
        assert result.coordinate == (12, 34)
        assert "[point" not in result.spoken_text.lower()
...
class TestParsePointTagFailClosed:
    """T0-3: malformed and truncated tags must be stripped, not spoken.

    annotations.py already did this for shape tags via _UNTERMINATED_TAG_RE;
    ai.py had no equivalent, so a truncated response leaked coordinates."""

    def test_unterminated_tag_stripped_with_trailing_content(self):
        from ai import parse_point_tag
        result = parse_point_tag("look here [POINT:120,40")
        assert result.coordinate is None
        assert result.spoken_text == "look here"
        assert "[POINT" not in result.spoken_text

    def test_unterminated_tag_eats_across_newlines(self):
        from ai import parse_point_tag
        result = parse_point_tag("look here [POINT:120,\n40 more junk")
        assert "[POINT" not in result.spoken_text
        assert result.spoken_text == "look here"

    def test_malformed_tag_recorded_for_diagnostics(self):
        """Stripping must not mean silently losing the evidence — the raw tag is
        surfaced on malformed_tags and logged by the pipeline worker."""
        from ai import parse_point_tag
        result = parse_point_tag("broken [POINT:not-a-coord]")
        assert result.malformed_tags == ("[POINT:not-a-coord]",)
        assert "[POINT" not in result.spoken_text

    def test_valid_tag_is_not_reported_as_malformed(self):
        from ai import parse_point_tag
        assert parse_point_tag("ok. [POINT:1,2:x]").malformed_tags == ()

    def test_point_none_is_not_reported_as_malformed(self):
        from ai import parse_point_tag
        assert parse_point_tag("ok. [POINT:none]").malformed_tags == ()

    def test_valid_tag_survives_alongside_malformed_one(self):
        """A valid tag must still be parsed even when a malformed one is present,
        and neither may reach spoken_text."""
        from ai import parse_point_tag
        result = parse_point_tag("a [POINT:garbage] b [POINT:7,8:btn] c")
        assert result.coordinate == (7, 8)
        assert result.malformed_tags == ("[POINT:garbage]",)
        assert "[POINT" not in result.spoken_text


class TestSpokenTextNeverContainsPointTag:
    """T0-3 HARD INVARIANT.

    spoken_text is handed straight to TTS. If a point tag survives in it, the
    user hears coordinates read aloud. No input may ever produce that, in any
    casing or state of truncation. This is the single most important guarantee
    in parse_point_tag and it is asserted exhaustively rather than by example."""

    @pytest.mark.parametrize("text", [
        # valid forms
        "[POINT:none]",
        "answer. [POINT:400,300:save]",
        "answer. [POINT:100,200]",
        "answer. [POINT:400,300:terminal:screen2]",
        "answer. [POINT:400,300:screen2]",
        "answer. [POINT:640 , 400:btn]",
        "answer. [POINT:50,60:button]  \n",
        # T0-3 mode 1 — signed
        "answer. [POINT:-3,50:edge]",
        "answer. [POINT:400,-12:tab]",
        "answer. [POINT:-8,-16:corner]",
        # T0-3 mode 2 — trailing prose / multiple / casing
        "answer. [POINT:400,300:save] hope that helps!",
        "a [POINT:1,2:x] b [POINT:3,4:y] c",
        "answer. [point:1,2:x]",
        "answer. [ POINT : 1 , 2 : x ]",
        # malformed + truncated
        "broken [POINT:garbage]",
        "broken [POINT:]",
        "broken [POINT:1]",
        "broken [POINT:a,b:c]",
        "look here [POINT:120,40",
        "look here [POINT:",
        "look here [point:120,\n40 junk",
        "a [POINT:garbage] b [POINT:7,8:btn] c",
        # no tag at all
        "just a plain response with no tag",
        "",
        "brackets in prose like the array index [0] must survive",
    ])
    def test_spoken_text_never_contains_point_marker(self, text):
        from ai import parse_point_tag
        spoken = parse_point_tag(text).spoken_text
        assert "[point" not in spoken.lower(), (
            f"point tag leaked into spoken_text for input {text!r} -> {spoken!r}"
        )

    def test_unrelated_brackets_are_preserved(self):
        """The strip must be narrow. Only POINT tags are removed — ordinary
        bracketed prose is part of the answer and must still be spoken."""
        from ai import parse_point_tag
        result = parse_point_tag("the array index [0] is first. [POINT:none]")
        assert "[0]" in result.spoken_text
...
class TestCreateAIClient:
    ...
    def test_routes_anthropic_prefix_to_anthropic_client(self, mocker):
        # T0-1: previously asserted model_id was stored VERBATIM as
        # "anthropic/model-sonnet-4-6". That was the bug — api.anthropic.com
        # rejects a namespaced slug, so every native-key request failed.
        # AnthropicClient now strips the routing namespace for the native
        # endpoint, matching what GeminiClient already did.
    ...
    def test_routes_claude_prefix_to_anthropic_client(self, mocker):
        """T0-1: a bare native id (no routing namespace) still reaches Anthropic.

        Replaces test_routes_model_prefix_to_anthropic_client, which exercised a
        `startswith("model")` branch — a find-and-replace fossil that made ANY
        id beginning with the literal string "model" route to Anthropic."""
        from ai import create_ai_client, AnthropicClient
    ...
    def test_bare_model_prefix_no_longer_routes_to_anthropic(self, mocker):
        """T0-1 regression guard: the removed fossil must not come back.

        An unrecognised prefix has to fail loudly. Silently routing it to
        Anthropic produced a confusing downstream API error instead of an
        actionable ValueError naming the supported prefixes."""
        import pytest as _pytest
        from ai import create_ai_client
        mocker.patch("ai.Anthropic")
        with _pytest.raises(ValueError, match="Unsupported MODEL_ID prefix"):
            create_ai_client(model_id="modelfoo/bar", api_key="test-key")

    def test_anthropic_openrouter_key_keeps_namespace_and_dots(self, mocker):
        """T0-1: the two endpoints disagree on slug format. OpenRouter wants the
        namespaced, dot-versioned slug; native wants bare and dash-versioned."""
        from ai import create_ai_client, AnthropicClient
        ...
        assert "openrouter" in mock_anthropic.call_args.kwargs["base_url"]
    ...
    def test_direct_google_key_routes_to_native_sdk(self, mocker):
        """T1-1: a direct Google key now routes to the NATIVE SDK client.

        This test previously asserted the key went to GeminiClient pointed at
        Google's OpenAI-COMPATIBILITY endpoint. That endpoint is still a shim: it
        exposes only the intersection of two API surfaces, which excludes structured
        spatial output, thinking budgets, grounding, and Agentic Vision. Routing a
        direct key to the native SDK is the entire point of T1-1.

        The OpenRouter path is unchanged and still serves single-key multi-vendor
        routing - asserted separately below.
        """
        from ai import create_ai_client
        ...
        assert client.model_id == "gemini-3.1-pro-preview"  # namespace stripped
        assert client.supports_structured_geometry() is True
        assert client.supports_thinking_budget() is True

    def test_new_format_google_key_routes_to_native_sdk(self, mocker):
        """T1-1: the newer AQ. key format must also be recognised.

        Verified against a real working key. The audit assumed only AIza existed;
        detecting a single prefix would have silently mis-routed every AQ. key to
        OpenRouter, where it fails auth.
        """
        from ai import create_ai_client
        from gemini_native import GeminiNativeClient
        client = create_ai_client(
            model_id="gemini/gemini-3-flash-preview",
            api_key="AQ.Ab8RN6JuFakeKeyForTesting",
        )
        assert isinstance(client, GeminiNativeClient)
...
class TestGeminiClientExtraCoverage:
    ...
    def test_history_image_blocks_are_carried_not_dropped(self, mocker):
        """EXPECTATION CHANGED (T2-4).

        This previously asserted that image blocks in history were silently dropped, which
        was accurate but was also the reason follow-ups like "what about that button you
        pointed at?" reached a model with no record of the previous screen.

        Images now convert to OpenAI `image_url` parts. A turn with no images still
        produces plain string content, so the default text-only path (HISTORY_IMAGE_COUNT
        defaults to 0) is byte-identical -- covered by
        test_history_without_images_still_uses_plain_string_content below.
        """
        from PIL import Image
        client, mock_instance = self._make_client_with_stream(mocker, [])
        history = [
        ...
        # [system, history_user_with_image, history_assistant, new_user]
        assert messages[1]["role"] == "user"
        parts = messages[1]["content"]
        assert isinstance(parts, list), "a turn with an image needs multipart content"
        assert {"type": "text", "text": "what is this"} in parts
        image_parts = [p for p in parts if p.get("type") == "image_url"]
        assert len(image_parts) == 1
        assert image_parts[0]["image_url"]["url"].startswith("data:image/jpeg;base64,")
        # An assistant turn carries no image, so it stays a plain string.
        assert messages[2] == {"role": "assistant", "content": "it's a cat."}

    def test_history_without_images_still_uses_plain_string_content(self, mocker):
        """T2-4 regression gate: the default path must be untouched."""
        from PIL import Image
        client, mock_instance = self._make_client_with_stream(mocker, [])
        history = [
        ...
        client.ask_stream(
            images=[(Image.new("RGB", (100, 60)), "screen 1")],
            transcript="and what else",
            history=history,
        )
        messages = mock_instance.chat.completions.create.call_args.kwargs["messages"]
        assert messages[1] == {"role": "user", "content": "what is this"}
        assert messages[2] == {"role": "assistant", "content": "a cat."}
