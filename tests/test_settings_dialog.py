...
class TestProviderCategoriesData:
    ...
    def test_anthropic_provider_exposes_model_picker(self):
        """T0-1: Anthropic must expose a model dropdown backed by ANTHROPIC_MODEL.

        This test previously asserted the OPPOSITE (`models == ()` and
        `model_setting == ""`), justified as "minimal UX: it uses the Anthropic
        default model". That reasoning did not survive contact with the default
        being wrong: with no picker, ANTHROPIC_MODEL was never written from the
        UI, so the broken placeholder default was the only value a GUI user
        could ever get. The picker is what makes the setting reachable.

        Model ids are the NATIVE dash-versioned form;
        ai._anthropic_model_for_endpoint adapts them for OpenRouter.
        """
        from settings_dialog import _PROVIDER_CATEGORIES
        llm = next(c for c in _PROVIDER_CATEGORIES if c.category_key == "LLM")
        ant = next(p for p in llm.providers if p.provider_id == "anthropic")
        assert ant.models, "Anthropic needs selectable models or the default is unreachable"
        assert ant.model_setting == "ANTHROPIC_MODEL"
        assert ant.models_editable, "user must be able to type a newer model id"
        for model in ant.models:
            assert "model-sonnet" not in model.model_id, "scrubbed placeholder returned"
            assert model.model_id.startswith("claude-"), "expected native bare id"
            assert "." not in model.model_id, "native ids are dash-versioned"

    def test_anthropic_default_model_is_offered_in_picker(self):
        """T0-1 drift guard: the shipped default must be one of the choices, or
        the dropdown opens on a value it cannot represent."""
        import config
        from settings_dialog import _PROVIDER_CATEGORIES
        llm = next(c for c in _PROVIDER_CATEGORIES if c.category_key == "LLM")
        ant = next(p for p in llm.providers if p.provider_id == "anthropic")
        assert config.DEFAULT_ANTHROPIC_MODEL in [m.model_id for m in ant.models]
