...
- [ ] 1. The pure policy function
- [ ] 1.1 Write the decision function with no I/O, no clock and no global state
  - Lists arrive as parameters with defaults, so the function never learns that settings exist
  - _Requirements: 1.1, 1.2, 1.3_
- [ ] 1.2 Put the policy in its own module, separate from the capture code
  - Mixing them would put a blocklist inside the highest-risk geometry code in the project
  - _Requirements: 1.4_
- [ ] 1.3 Order the checks: enabled, then application name, then window title
  - _Requirements: 1.5, 1.6_
- [ ] 1.4 Fail open on the unknown sentinel and on an empty name, with the reasoning in the module
...
- [ ] 1.5 Keep reason strings user-presentable, with no regex, path or executable name in them
  - The toast is on screen and may itself be captured in a screenshot or a recording
  - _Requirements: 9.6_

- [ ] 2. The blocklists
- [ ] 2.1 Build the application list from the machine, not from memory
...
- [ ] 2.2 Match application names by exact lowercase basename
  - _Requirements: 3.1_
- [ ] 2.3 Write the title patterns in four labelled groups
  - Authentication, finance, secret-bearing filenames, private browsing
  - _Requirements: 3.3, 3.4_
- [ ] 2.4 Match titles case-insensitively as a search
  - _Requirements: 3.5_
- [ ] 2.5 Make user lists additive to the defaults
  - So pinning one extra application cannot accidentally discard the built-in list
  - _Requirements: 3.6_
- [ ] 2.6 Discard an invalid user pattern and keep the rest
...
- [ ] 3.1 Replace the bare word match for "password" with three context-aware patterns
...
- [ ] 3.2 Fix the environment-file pattern to catch the extension after a filename
...
- [ ] 3.3 Write a must-not-suppress table alongside the must-suppress table
  - A guard tested only for what it catches has not been checked for what it wrongly catches
  - _Requirements: 4.6_

- [ ] 4. The single capture choke point
- [ ] 4.1 Collapse every capture call site into one guarded helper
...
- [ ] 4.2 Evaluate the guard against the application recorded at press time
...
- [ ] 4.3 Skip the overlay hide-and-show cycle entirely on a suppression
  - No flicker for a capture that is not happening
  - _Requirements: 6.7_
- [ ] 4.4 Restore the overlay in a `finally` on the permitted path
  - _Requirements: 6.7_
- [ ] 4.5 Extend the guard to the re-point path
  - Re-pointing must not become a way to photograph a password manager
  - _Requirements: 9.4_

- [ ] 5. Voice-only turn handling
- [ ] 5.1 Return an empty list rather than raising, and document that it is not an abort
  - The user asked a question and deserves an answer even if Nimbus must answer it blind
  - _Requirements: 6.1, 6.2_
- [ ] 5.2 Tell the model plainly that the screen was withheld
  - Otherwise it answers as though it can see and describes a screen it was never shown
  - _Requirements: 6.3_
- [ ] 5.3 Discard any coordinate returned on a suppressed turn
  - A model given no image can still emit one; placing a pointer from it would be pure invention
  - _Requirements: 6.4_
- [ ] 5.4 Discard annotations while still stripping their tags from the spoken text
  - _Requirements: 6.5_
- [ ] 5.5 Skip the grid locator on a suppressed turn
  - _Requirements: 6.6_
- [ ] 5.6 Toast the reason and state that the question is being answered without the screen
  - _Requirements: 6.8_

- [ ] 6. Settings and defaults
- [ ] 6.1 Default the guard to on, and record the deviation with its justification
...
- [ ] 6.2 Persist an explicit on or off rather than deleting the key
  - This setting defaults on, so treating absent as off would silently disable a privacy feature the
    user believes is active
  - _Requirements: 7.4_
- [ ] 6.3 Add the two extension settings, and mark all three restart-required
  - _Requirements: 3.6_
- [ ] 6.4 Include all three in the local-data wipe, restoring the on default
  - A wipe must not silently weaken privacy
  - _Requirements: 7.3_
- [ ] 6.5 Add the Privacy group to the Settings form with its explanation
  - _Requirements: 7.1_

- [ ] 7. In-memory counting
- [ ] 7.1 Count a suppression once, at the choke point
  - _Requirements: 5.1, 5.2_
- [ ] 7.2 Trim the in-memory list to the reporting window
  - _Requirements: 5.5_

- [ ] 8. Durable counting
- [ ] 8.1 Add the suppressions table to the existing database, purely additively
  - That database already exists, is already in the right folder, and is already pruned
  - _Requirements: 5.3_
- [ ] 8.2 Record durably inside a try/except at the choke point
  - A counter must never cost the user their answer
  - _Requirements: 5.4_
- [ ] 8.3 Read the count for a rolling seven-day window, in-memory as the fallback
...
- [ ] 9. Surfacing the count
- [ ] 9.1 Show the count on the Home page
  - _Requirements: 5.6_
- [ ] 9.2 Render a placeholder rather than zero when the count cannot be read
  - An unmeasured zero is a false claim, and this is the number whose whole value is being an
    observation
  - _Requirements: 5.7_
- [ ] 9.3 Add the always-visible state indicator to the navigation rail
  - _Requirements: 5.8_
- [ ] 9.4 Use the danger colour when the guard is off, not a gentler warning colour
...
- [ ] 10. Exhaustive tests
- [ ] 10.1 Assert the must-suppress and must-not-suppress tables
  - _Requirements: 4.6_
- [ ] 10.2 Assert the function is pure
  - _Requirements: 1.1_
- [ ] 10.3 Assert every default application entry is lowercase
  - An uppercase entry could never match, so it would be a silent gap
  - _Requirements: 3.1_
- [ ] 10.4 Assert every default title pattern compiles
  - A pattern that cannot compile protects nothing
  - _Requirements: 3.3_
- [ ] 10.5 Assert graceful degradation in both directions for malformed user patterns
  - _Requirements: 3.7_
- [ ] 10.6 Assert reason strings are safe to display
  - _Requirements: 9.6_
- [ ] 10.7 Integration: a suppressed turn answers, emits no coordinate, writes no screenshot
  - _Requirements: 6.1, 6.2, 6.4_
- [ ] 10.8 Assert the re-point path respects the guard
  - _Requirements: 9.4_

- [ ] 11. Tests and verification
- [ ] 11.1 Full suite green with the dotenv neutralisation, zero regressions
- [ ] 11.2 `--selftest` prints `SELFTEST OK`
- [ ] 11.3 Manual: real password manager in front — toast appears, answer is spoken, no screenshot
      in the diagnostics folder, Home count increases
- [ ] 11.4 Manual: guard off restores the previous behaviour exactly
- [ ] 11.5 Write the tests for this feature - 289 declared functions
