...
- [x] 1. The signing core
- [x] 1.1 Sign with the runtime's own crypto and **zero** dependencies
  ...
  - _Requirements: 8.6_
- [x] 1.2 Serialise the payload over **sorted keys with no whitespace**, omitting empties
...
- [x] 1.3 Match the client's token format exactly, and record why it is not a standard web token
  - The standard carries an algorithm field, and algorithm negotiation is where those libraries get broken
  - _Requirements: 8.1, 8.2_
- [x] 1.4 Read the private key from the environment and validate its decoded length
  - Rather than trusting it and failing at signing time with an opaque error
  - _Requirements: 8.7_
- [x] 1.5 Add a verification function using the public half
  - So a deployment with a mismatched pair is caught by a health check rather than by a tester trying to
    activate
  - _Requirements: 8.8_
- [x] 1.6 Add the cross-language contract test on the exact bytes
  ...
  - _Requirements: 8.9_
- [x] 1.7 Generate licence keys from a secure source over an unambiguous alphabet
...
- [x] 1.8 Cap the token lifetime below the billing period
...
- [x] 1.9 Set the default seat count with its justification
...
- [x] 2. The schema
- [x] 2.1 Define exactly seven models, and record why no more
...
- [x] 2.2 Use Postgres rather than a file-backed database
  - This runs where the filesystem is ephemeral, so a file would be wiped on every deploy, taking the
    account list with it
  - _Requirements: 1.3_
- [x] 2.3 Cascade deletes from an account and from a licence
  - _Requirements: 1.4_
- [x] 2.4 Add the append-only audit trail
  - When someone says their licence stopped working, the answer is in there
  - _Requirements: 1.5_
- [x] 2.5 Make the licence-and-device pairing unique
  - So a double request cannot create two rows for one machine
  - _Requirements: 11.9_
- [x] 2.6 Key the trial on the **device**, with no address column and an optional owner
...
- [x] 2.7 Give the manual payment a status and a review timestamp
  - So its lifecycle is visible
  - _Requirements: 13.6_
- [x] 2.8 Configure both a pooled and a direct database address
  - Schema operations need a direct connection
  - _Requirements: 20.4_

- [x] 3. Accounts, sessions and single-use secrets
- [x] 3.1 Hash passwords with the runtime's own memory-hard function, salted per account
...
- [x] 3.2 Declare the maximum memory parameter **explicitly** — shipped defect
...
- [x] 3.6 Set the relaxed same-site policy, **not** the strict one
  ...
  - _Requirements: 3.3_
- [x] 3.7 State the revocation trade rather than hiding it
...
- [x] 3.8 Require the signing secret to meet a minimum length, and error on its absence
  - _Requirements: 3.6_
- [x] 3.9 Add the three-valued session state — shipped defect
...
- [x] 3.10 Branch every page on all three states, and add the endpoint that clears a stale cookie
  - So the loop is breakable from the client
  - _Requirements: 4.4, 4.5_
- [x] 3.11 Trust the cookie when the account lookup **throws**
...
- [x] 3.12 Use one model for verification codes, sign-in links and password resets
...
- [x] 3.13 Store only the hash, salted with the account identifier for codes
...
- [x] 3.14 Put the attempt counter **on the row**
  - Six digits is a million possibilities: plenty against a person, nothing against a script. So the code
    ...
    not share it
  - _Requirements: 5.6_
- [x] 3.15 Delete any earlier unused code for the same purpose at issue time
  ...
  - _Requirements: 5.7_
- [x] 3.16 Return distinct outcomes for correct, wrong, expired and too-many; consume on success
  - _Requirements: 5.8, 5.9_
- [x] 3.17 Make links single-use and purpose-bound
  - Rejected if unknown, expired, already used, or issued for a different purpose
  - _Requirements: 5.10_
- [x] 3.18 Record why both a code and a link exist
  ...
  - _Requirements: 5.11_
- [x] 3.19 Store the failed-attempt counter and lockout on the account, checked before any comparison
...
- [x] 4. Licence issuance
- [x] 4.1 Make issuance **one** function shared by all three rails
  - Three copies of "issue a licence" is three chances to issue two
  - _Requirements: 10.1, 10.2_
- [x] 4.2 Make issuance idempotent
  ...
  - _Requirements: 10.3_
- [x] 4.3 Extend rather than replace an existing active licence, moving the period end only forward
  - That path is also how a renewal arrives
  - _Requirements: 10.4, 10.5_
- [x] 4.4 Record an audit event on creation
  - _Requirements: 10.6_
- [x] 4.5 Always readmit a device already on the licence, **even at the seat limit**
...
- [x] 4.6 Refuse a new device at the limit, and floor the seat total at one
  - So a misconfigured zero does not lock out everybody
  - _Requirements: 11.5, 11.6_
- [x] 4.7 Mark a released device inactive rather than deleting it
  - So the audit trail survives
  - _Requirements: 11.7_
- [x] 4.8 Take the device identifier as the salted hash the desktop application computes
  - Never a raw hardware identifier
  - _Requirements: 11.8_
- [x] 4.9 Return the **existing** expiry for a device that already has a trial
...
- [x] 4.10 Never overwrite an existing trial owner
  ...
  - _Requirements: 12.6_
- [x] 4.11 Truncate the trial expiry to the stored precision
  - Otherwise the token issued on the first request and the one issued on a reinstall disagree about the
    same trial's expiry
  - _Requirements: 12.9_
- [x] 4.12 Record an audit event when a new trial starts
  - _Requirements: 12.10_

- [x] 5. Desktop endpoints
- [x] 5.1 Add activation, refresh, deactivation and trial endpoints
  - _Requirements: 10.1_
- [x] 5.2 Add the register, verify and login endpoints the desktop flow uses
  - Verification and trial start in one call, because from the user's side it is one action
  - _Requirements: 5.11_
- [x] 5.3 Return the licence key alongside the token on the login route
  - So the client caches the key for revalidation and never stores the password
  - _Requirements: 10.1_

- [x] 6. Card checkout
- [x] 6.1 Create a checkout session and route the webhook through the shared issuance function
  - _Requirements: 10.1_
- [x] 6.2 Verify the webhook signature before acting on any event
  - _Requirements: 10.1_
- [x] 6.3 Let the success page ask for the licence before the webhook lands
  - Which is one of the two reasons issuance has to be idempotent
  - _Requirements: 10.3_

- [x] 7. The manual payment rail
- [x] 7.1 Record the research behind the automated alternative, with sources cited
  - The provider does have an official gateway, it is not self-serve, and it requires a merchant
    ...
    a card processor
  - _Requirements: 13.7_
- [x] 7.2 Offer the hosted checkout only where credentials are configured
...
- [x] 7.3 Sort the parameters before hashing and validate the key length
...
- [x] 7.4 Use a deliberately set local-currency amount rather than a converted one
  - A rate that moves under a tester mid-checkout is how a payment ends up short and rejected
  - _Requirements: 14.4_
- [x] 7.5 Select the test or live endpoint by configuration
  - _Requirements: 14.5_
- [x] 7.6 Record a pending payment and **mint nothing**
  - There is no signing secret on that rail, so the callback carries no proof, and a payment claim is not a
    payment
  - _Requirements: 13.1, 13.2_
- [x] 7.7 Present the manual path as manual
...
- [x] 8. Administrative approval
- [x] 8.1 Make approval an explicit action that calls the shared issuance function
  - The only path from the manual rail to a licence
  - _Requirements: 13.3_
- [x] 8.2 Build the review interface over pending payments
  - _Requirements: 13.6_

- [x] 9. The error map and status conventions
- [x] 9.1 Put every user-facing failure in one map with a machine code and a sentence
...
- [x] 9.2 Record the account-existence decision as an argued choice
...
- [x] 9.3 Keep the reset path silent about whether an address exists
  - Confirming one to an unauthenticated stranger has no upside at all
  - _Requirements: 15.6_
- [x] 9.4 Add the client-side helper that turns any response into a message
  ...
  - _Requirements: 15.7_
- [x] 9.5 Say when nothing was charged
  - _Requirements: 15.8_
- [x] 9.6 Return the payment-required status for a missing subscription, not the forbidden one
...
- [x] 9.7 Partition every error path: user state is 4xx, our failure is 5xx
  - _Requirements: 16.3, 16.4_
- [x] 9.8 Carry both the code and the sentence in every error response
  - So the client can show the message without knowing the code
  - _Requirements: 16.5_

- [x] 10. Health check
- [x] 10.1 Sign a token and verify it with the configured public half on every call
  - So a deployment with a mismatched pair is caught by a check rather than by a tester trying to activate
  - _Requirements: 17.1, 17.2_
- [x] 10.2 Report database reachability, and disclose no secret
  - _Requirements: 17.3, 17.4_

- [x] 11. Pages
- [x] 11.1 Build the landing, signup, sign-in, reset, account, pay and administration pages
  - _Requirements: 4.4_
- [x] 11.2 Show the licence key, the bound devices and the seat count on the account page
  - _Requirements: 11.1_
- [x] 11.3 Offer only the payment methods the configuration can honour
  - _Requirements: 13.9_
...
- [x] 12.2 Pin a single deployment region
  ...
  - _Requirements: 20.1_
- [x] 12.3 Make the apex the production target, with the alternative host redirecting permanently
...
- [x] 12.4 Pin dependency versions exactly rather than by range
  - A range is how an advisory-fixed version silently regresses
  - _Requirements: 19.4_
- [x] 12.5 Resolve a security advisory by an explicit version bump, recording the advisory identifier
  - _Requirements: 19.5_
- [x] 12.6 Document the build configuration's error-suppressing options as a **release valve**
  - So nobody reads their presence as permission to skip the check
  - _Requirements: 19.3_
- [x] 12.7 Record the earlier standalone service directory as **superseded**, with its tests still passing
  - So its presence is not mistaken for rot, and its status is not left ambiguous
  - _Requirements: 21.1, 21.2, 21.3_

- [x] 13. Tests and verification
- [x] 13.1 Explicit typecheck passing — the real gate, run separately from the build — shipped defect
...
- [x] 13.2 The signing test file green, including the cross-language byte contract
- [x] 13.3 Health endpoint returning a successful sign-and-verify against the live deployment
- [x] 13.4 Manual: complete a card activation, confirm **one** key arrives rather than three
- [x] 13.5 Manual: activate on two machines, confirm the seat count; try a third, confirm the refusal names the action
- [x] 13.6 Manual: submit a manual payment, confirm no licence appears until approval
- [x] 13.7 Manual: delete an account holding a live cookie, confirm the sign-in page recovers rather than looping
- [x] 13.8 Manual: request a reset for an address that does not exist, confirm the response is identical to one that does
- [x] 13.9 Write the tests for this feature - 14 declared functions
