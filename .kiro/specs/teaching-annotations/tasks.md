...
- [x] 1. Click-through overlay
- [x] 1.1 Create one frameless always-on-top window per physical monitor
  - A single spanning window renders wrong on mixed DPI; per-monitor is what makes the
    "islands of screens" case correct
  - _Requirements: 1.1, 1.7_
- [x] 1.2 Apply the Win32 extended styles after `show()`, OR-ing rather than assigning
  - The native handle does not exist before `show()`; assigning would wipe the toolkit's own flags
  - _Requirements: 1.2, 1.3_
- [x] 1.3 Force a frame recalculation so the style change takes effect immediately
  - Without it the new styles do nothing until the window is resized or moved
  - _Requirements: 1.4_
- [x] 1.4 Raise on style-change failure, including the Win32 error detail
  - _Requirements: 1.5_
- [x] 1.5 Use window attributes for transparency, never a stylesheet or a window opacity
...
- [x] 1.6 Pin the bit pattern with a test asserting both the expression and the literal
  - _Requirements: 1.3_

- [x] 2. Coordinate transforms
- [x] 2.1 Write the physical-to-local-logical transform using the per-screen ratio
  - _Requirements: 2.1_
- [x] 2.2 Match a monitor descriptor to a screen by physical geometry, primary as fallback
  - _Requirements: 2.2_
- [x] 2.3 Write the shape transform: positions transform, lengths only scale
  - _Requirements: 2.3, 2.4_
- [x] 2.4 Keep both transforms pure so they test with no toolkit
  - _Requirements: 2.6_
- [x] 2.5 Test at 100% and 200% scaling, on primary and offset screens
  - _Requirements: 2.1, 2.2, 2.3_

- [x] 3. The pointer
- [x] 3.1 Trace the silhouette from the brand artwork with a tool, not by hand
...
- [x] 3.2 Implement the Bézier flight with a distance-scaled clamped duration
  - _Requirements: 3.3, 3.4_
- [x] 3.3 Suppress tangent rotation; the tip stays on target through the flight
  - _Requirements: 3.5_
- [x] 3.4 Drive position by eased progress and the scale pulse by linear progress
  - So the 1.3× peak lands at the true mid-arc rather than the eased midpoint
  - _Requirements: 3.6_
- [x] 3.5 Apply the scale about the tip
  - _Requirements: 3.7_
- [x] 3.6 Snap to the target and reset scale at completion
  - Defensive: the animation sometimes emits its final value slightly early
  - _Requirements: 3.8_
- [x] 3.7 Fly back to the mouse after the dwell and resume spring-following it
  - _Requirements: 3.9_
- [x] 3.8 Add the idle breathing pulse within a restrained range
  - _Requirements: 3.10_
- [x] 3.9 Derive the fill's highlight and lower edge from the state accent
  ...
  - _Requirements: 7.2_
- [x] 3.10 Stroke a black outline under the fill at a wider pen
...
- [x] 4. Interaction state visuals
- [x] 4.1 Define one central state-to-accent mapping, sourced from the design system
  - _Requirements: 4.1_
- [x] 4.2 Build the audio-reactive waveform with a dead zone and a non-flat floor
  - _Requirements: 4.2, 4.3, 4.4_
- [x] 4.3 Build the thinking spinner with a comet tail
  - _Requirements: 4.5_
- [x] 4.4 Keep the listening state green, and record why
...
- [x] 4.5 Enforce mutual exclusion between pointer, waveform and spinner
  - _Requirements: 4.9_

- [x] 5. Tag grammar and shapes
- [x] 5.1 Define the shapes as immutable value objects
  - _Requirements: 5.6_
- [x] 5.2 Write case-insensitive, whitespace-tolerant regexes for each shape
  - The prompt asks for lowercase prose, so a lowercase tag must both parse **and** strip
  - _Requirements: 6.4_
- [x] 5.3 Strip every complete tag from the spoken text
  - _Requirements: 6.5_
- [x] 5.4 Add the fail-closed strip for an unterminated tag and everything after it
  - _Requirements: 6.6_
- [x] 5.5 Centralise the keyword list so both patterns read from one source
  - The list was written twice; forgetting the second would let a truncated tag's coordinates be
    read aloud
  - _Requirements: 6.7_
- [x] 5.6 Drop malformed tags silently rather than raising
  - _Requirements: 6.8_
- [x] 5.7 Return shapes in order of appearance
  - _Requirements: 6.9_

- [x] 6. Rendering annotations
- [x] 6.1 Paint highlights in an unconditional first pass, before everything else
  - Not in list order: the model controls that order and must not be able to break the visual
  - _Requirements: 5.4_
- [x] 6.2 Add the fade-in, hold and fade-out envelope before the automatic clear
  - _Requirements: 5.7_
- [x] 6.3 Clear annotations at the start of every turn
  - So stale shapes never survive a no-speech, cancelled or errored previous turn
  - _Requirements: 5.8_
- [x] 6.4 Route annotations to one overlay and clear the others
  - _Requirements: 2.2_

- [x] 7. Richer vocabulary (`T3-5`)
- [x] 7.1 Add the rectangle shape with a centre property
  ...
  - _Requirements: 5.2_
- [x] 7.2 Add the highlight shape with inverted geometry
  ...
  - _Requirements: 5.3_
- [x] 7.3 Add the 1-based numbered step badge
  - _Requirements: 5.5_
- [x] 7.4 Extend the tag grammar and the transform to all three
...
- [x] 8. Design system extraction
- [x] 8.1 Define the five-step elevation ramp with a consistent tint
  ...
  - _Requirements: 7.3_
- [x] 8.2 Define the spacing scale as the only permitted source of spacing
  - _Requirements: 7.4_
- [x] 8.3 Define exactly one accent hue and its derived states
  - _Requirements: 7.5_
- [x] 8.4 Write the contrast helpers and audit every text colour
  - Caught the muted colour at 3.49:1 against a 4.5:1 requirement — failing for exactly the small
    ...
    retried
  - _Requirements: 7.6_
- [x] 8.5 Make the disabled colour deliberately fail, with a test saying so
  - _Requirements: 7.7_
- [x] 8.6 Generate the stylesheet from the constants and assert no literal colour survives
  - _Requirements: 7.8_
- [x] 8.7 Exclude the overlay from the stylesheet; have it consume constants directly
  - _Requirements: 7.9_
- [x] 8.8 Re-theme the overlay's state palette through the design system
  - Was a one-dict change in the overlay; now a one-dict change in the design system, which is the
    right place for it
  - _Requirements: 4.1, 7.1, 7.2_

- [x] 9. Shading and grain
- [x] 9.1 Add the top-edge highlight to every raised surface
  - The single highest-impact technique: the difference between a card reading as an object and
    reading as a hole
  - _Requirements: 9.1_
- [x] 9.2 Tint each elevation step rather than only lightening it
  - _Requirements: 9.2_
- [x] 9.3 Add the ambient accent bloom, offset to zero, on static elements only
  - _Requirements: 9.3, 9.4_
- [x] 9.4 Paint the bloom with a gradient on any surface that repaints per frame
  - _Requirements: 9.5_
- [x] 9.5 Add the window-level noise overlay, varying alpha rather than colour
...
- [x] 9.6 Fix the seed so the tile is byte-identical between runs
  - A grain layer that changed per launch would make screenshots and visual diffs useless
  - _Requirements: 9.8_
- [x] 9.7 Make the overlay mouse-transparent, one instance per window
  - Verified by hit-testing a button's centre: without the flag the click resolves to the grain widget
  - _Requirements: 9.9_
- [x] 9.8 Give cards no drop shadow
  - _Requirements: 9.10_

- [x] 10. Motion and reduced motion
- [x] 10.1 Define micro, standard, entrance and exit durations plus a cap
  - _Requirements: 8.1_
- [x] 10.2 Make every exit faster than its entrance, and assert it
  - The most common mistake in hand-rolled motion: equal durations make dismissal feel sluggish
  - _Requirements: 8.2_
- [x] 10.3 Prefer animating opacity and position over layout properties
  - _Requirements: 8.3_
- [x] 10.4 Keep all animation on the main thread
  - _Requirements: 8.4_
- [x] 10.5 Honour the system reduced-motion preference with an explicit override
  - _Requirements: 8.5_
- [x] 10.6 Collapse every duration to zero through one helper
  - One call site per animation rather than a conditional
  - _Requirements: 8.6_
- [x] 10.7 Verify a zero-duration animation still emits completion, and pin it
  - Cleanup logic hangs off that signal; without the guarantee, disabling motion silently breaks it
  - _Requirements: 8.7_
- [x] 10.8 Cache the preference check once and fail open
  - _Requirements: 8.8_

- [x] 11. Structured channel convergence
- [x] 11.1 Map each structured tool call onto the same shape objects
  - _Requirements: 6.2, 6.3_
- [x] 11.2 Convert a bare point into a small circle so it is visible as an annotation
  - The pointer path uses the coordinate directly; this is the annotation representation
  - _Requirements: 6.3_
- [x] 11.3 Test that identical geometry through both channels yields equal objects
  - _Requirements: 6.3_

- [x] 12. Brand loader
- [x] 12.1 Load every brand asset through one module
  - Five call sites previously meant five sizes and five amounts of accidental padding
  - _Requirements: 10.1_
- [x] 12.2 Crop to the alpha bounding box before scaling
  ...
  - _Requirements: 10.2_
- [x] 12.3 Cache the alpha scan per asset, not per size
  ...
  - _Requirements: 10.3_
- [x] 12.4 Return an empty image rather than raising on a missing asset
  - _Requirements: 10.4_
- [x] 12.5 Make brand labels mouse-transparent
  - A click-eating label put a dead spot in the title bar and made the chat panel undraggable
  - _Requirements: 10.5_
- [x] 12.6 Assert the artwork bounding boxes in tests
  - _Requirements: 10.6_

- [x] 13. Tests and verification
- [x] 13.1 Full suite green with the dotenv neutralisation, zero regressions
- [x] 13.2 `--selftest` prints `SELFTEST OK`
- [x] 13.3 Visual review through the preview harness, which writes nothing
- [x] 13.4 Manual check on a mixed-DPI monitor pair: the pointer lands correctly on both
- [x] 13.5 Manual check that click-through works — the interface is unusable if it does not
- [x] 13.6 Write the tests for this feature - 219 declared functions
