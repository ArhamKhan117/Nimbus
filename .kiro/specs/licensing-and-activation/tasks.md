...
- [x] 1. Token verification (`S-10`)
- [x] 1.1 Define the token as a base64url payload and signature separated by one dot
...
- [x] 1.2 Verify against a public key embedded in the build, so verification needs no network
  - _Requirements: 2.3_
- [x] 1.3 Fail closed with no configured key, and reconstruct base64 padding on decode
  - The halves are stored unpadded
  - _Requirements: 2.6, 2.10_
- [x] 1.4 Raise on any suspect input, with a message fit to show a user
  - A tampered token is indistinguishable from a corrupt one from here, and both mean "do not trust"
  - _Requirements: 2.8, 2.9_
- [x] 1.5 Reject claims that do not decode to a mapping
  - _Requirements: 2.11_
- [x] 1.6 Resolve the key from the environment first, then the baked constant
  ...
  - _Requirements: 2.3_
- [x] 1.7 Keep the private half out of the repository, and add the test that asserts it
...
- [x] 1.8 Record the honest position in the module docstring
...
- [x] 2. Device identity
- [x] 2.1 Build the identifier as a salted hash of three values, truncated
...
- [x] 2.2 Read the installation identifier from the operating system's own registry value
  - Not a hardware serial, and suppress the console window so a windowed build does not flash a terminal
  - _Requirements: 3.4, 3.8_
- [x] 2.3 Include the volume serial, which changes on a reformat
  - _Requirements: 3.5_
- [x] 2.4 Fall back to the hostname when both lookups fail, and to a sentinel when nothing is available
  - Weakens the binding on an unusual machine rather than blocking a legitimate user, which is the right
    way round
  - _Requirements: 3.6, 3.7_

- [x] 3. Storage
- [x] 3.1 Store in the per-user credential store as the primary location
  - Already a dependency, already encrypted per user, and a licence file on disk is trivially copied
    between machines
  - _Requirements: 4.1, 4.2_
- [x] 3.2 Measure the credential store's real ceiling and fall back to a file above it
...
- [x] 3.3 Read from the credential store first, then the file; clear both
  - _Requirements: 4.6, 4.7_
- [x] 3.4 Swallow every storage failure and report success as a boolean
  - _Requirements: 4.8_

- [x] 4. The trial and the state read
- [x] 4.1 Make the trial a signed token on the same verification path
  - Differs only in its claims; the trial is not a special case in verification
  - _Requirements: 5.1_
- [x] 4.2 Key the trial on the device identifier server-side
  ...
  - _Requirements: 5.2_
- [x] 4.3 Keep two independent first-run records, and take the earliest
...
- [x] 4.4 Round day counts **up**, and clamp at the nominal period
...
- [x] 4.5 Apply the same rounding to the token-derived count
  ...
  - _Requirements: 6.5_
- [x] 4.6 Expose both a trial-specific and a kind-agnostic day count
...
- [x] 4.7 Give account holders a countdown too
...
- [x] 4.11 Report the offline grace only when it is meaningful
  - A permanent grace line is noise on a machine that has been online all week
  - _Requirements: 7.4_
- [x] 4.12 Include the grace period in the run decision
  - The common reason for an expired token is a laptop that has not been online, not a lapsed card
  - _Requirements: 7.5, 7.6_
- [x] 4.13 Vary the detail string by kind and by expiry
  - A lapsed subscription, an ended trial and a healthy licence each read differently
  - _Requirements: 7.7_

- [x] 5. The network layer
- [x] 5.1 Resolve the service address from the environment, then the baked constant, then a default
  - _Requirements: 9.1_
- [x] 5.2 Accept a comma-separated list, with a single value behaving exactly as before
...
- [x] 5.3 Advance to the next candidate **only** on unreachability
...
- [x] 5.4 Use a short timeout
  - Activation is interactive, and a hung request reads as a broken application
  - _Requirements: 9.11_
- [x] 5.5 Distinguish a server error from a rejection, and reject a non-mapping body
  - _Requirements: 9.12, 9.13_
- [x] 5.6 **Follow redirects** — shipped defect, found in production
...
- [x] 6. Activation routes
- [x] 6.1 Activate by licence key
  - _Requirements: 10.1_
- [x] 6.2 Activate by the email and password the tester registered with
...
- [x] 6.3 Register from inside the application, then verify a code that also starts the trial
...
- [x] 6.4 Record the anonymous-to-identified trial trade honestly
...
- [x] 6.5 Add the browser signup route as an escape hatch
...
- [x] 6.6 Verify the returned token **before** storing it, on every route
...
- [x] 7.2 **Never** clear a good licence because the network was down
...
- [x] 7.3 Make revalidation a no-op with no cached key
  - _Requirements: 8.8_
- [x] 7.4 Clear the licence, key, trial token and last-validated record on sign-out
  - And leave the first-run record alone, because signing out is not a way to restart the trial clock
  - _Requirements: 11.1, 11.2_
- [x] 7.5 Release the seat, clearing locally **even if the service call fails**
...
- [x] 7.6 Re-run the gate after an in-application sign-out — shipped defect
...
- [x] 8. The activation dialog
- [x] 8.1 Build the modal, blocking flow with retry, offline, register, sign in and start trial
  - Never silently degrade: a legitimate user must never be left guessing
  - _Requirements: 12.3, 12.4_
- [x] 8.2 Show the trial's day count and the plan detail from the state, not from constants
  - _Requirements: 6.8_

- [x] 9. The startup gate
- [x] 9.1 Place the gate after the application object and **before** the listener and the microphone
  - An unlicensed instance should consume no devices and register no global hooks
  - _Requirements: 12.1, 12.2_
- [x] 9.2 Extract the gate into its own function with **all imports local to it** — shipped defect
...
- [x] 9.3 Catch any gate exception, record it to a **file**, and start anyway
...
- [x] 9.4 Add the regression test that injects an exception at each import and call inside the gate
  - Asserting the application still starts and a record lands on disk
  - _Requirements: 12.8, 12.9_
- [x] 9.5 Add the static assertion that the gate call precedes the listener and the microphone
  - The ordering is the whole point of having a gate
  - _Requirements: 12.1_

- [x] 10. Build chain and the baked key
- [x] 10.1 Write the build-time constants into a generated, git-ignored module
  - Public half only, plus the service address
  - _Requirements: 13.1, 13.2, 13.3_
- [x] 10.2 Treat a missing generated module as normal, and never raise reading a baked constant
  - That is a development checkout, and the environment variable covers it
  - _Requirements: 13.4, 13.5_
- [x] 10.3 Let the environment override the baked constant
  - So the test suite can sign with a throwaway key and a staging service needs no rebuild
  - _Requirements: 13.6_
- [x] 10.4 Make the build script report whether a key is present
  - Rather than leaving its absence to be discovered by the first person who tries to activate
  - _Requirements: 2.7_
- [x] 10.5 Add the tool that issues a licence locally
  - So the activation path can be exercised without the live service
  - _Requirements: 13.7_
- [x] 10.6 Verify the produced bundle rather than assuming it
  - Including that the licence modules are present
  - _Requirements: 13.8_
- [x] 10.7 Register every lazily imported module in **both** the bundler list and the selftest list
  ...
  - _Requirements: 13.9_
- [x] 10.8 Write no auto-start entry in the installer
  ...
  - _Requirements: 14.7_
- [x] 10.9 Point the installer's update link at the releases page, not a website path
...
- [x] 11. Release publishing
- [x] 11.1 Publish installers to a repository readable without credentials
...
- [x] 11.2 Use the platform-injected token rather than a stored secret
  ...
  - _Requirements: 14.3_
- [x] 11.3 Check the exit status of every publishing call
  - _Requirements: 14.4_
- [x] 11.4 Read the published asset list back and confirm the installer is present
...
- [x] 12.2 Keep the version comparison a pure function
  - Testable without a network
  - _Requirements: 15.5_
- [x] 12.3 Keep a failed check silent and non-blocking
  - _Requirements: 15.6_

- [x] 13. Tests and verification
- [x] 13.1 Full suite green with the dotenv neutralisation, zero regressions
- [x] 13.2 `--selftest` prints `SELFTEST OK`, with every licence module in the runtime list
- [x] 13.3 Set an explicit maximum memory parameter on the server-side password hash — shipped defect
...
- [x] 13.4 Cross-language contract test on the token bytes
...
- [x] 13.5 Manual: activate with a key on a clean machine
- [x] 13.6 Manual: activate the same key on a second machine, confirm the seat count moves
- [x] 13.7 Manual: activate on a third machine, confirm refusal
- [x] 13.8 Manual: disconnect the network, confirm Nimbus still starts
- [x] 13.9 Manual: advance past the grace period, confirm it stops
- [x] 13.10 Manual: sign out, confirm the gate reappears
- [x] 13.11 Manual: register, receive the code, verify, confirm the trial reads the full period
- [x] 13.12 Write the tests for this feature - 134 declared functions
