...
- [ ] 1. Stabilise the provider abstraction (Tier 0)
- [ ] 1.1 Fix the Anthropic provider, which failed on every request (`T0-1`)
...
- [ ] 1.2 Reconcile three conflicting provider defaults (`T0-2`)
  - The audit said two call sites; verification found **three**. A drift-guard test now counts them
  - _Requirements: 1.1_
- [ ] 1.3 Fix three silent-failure modes in coordinate tag parsing (`T0-3`)
  ...
  - _Requirements: 2.7_
- [ ] 1.4 Centralise the endpoint decision in one function
  - _Requirements: 1.4_
- [ ] 1.5 Improve the startup provider log (`T0-4`)
...
- [ ] 2. Native client (`T1-1`)
- [ ] 2.1 Add the native client as a distinct provider, not auto-detection inside the existing one
...
- [ ] 2.2 Build the SDK client lazily on first use, behind an injectable factory
  - _Requirements: 1.6_
- [ ] 2.3 Route a direct key to the native path and an OpenRouter key to the compatibility shim
  - Two recognised key prefixes, one of them confirmed against a working key rather than guessed
  - _Requirements: 1.4_

- [ ] 3. Structured geometry (`T1-2`)
- [ ] 3.1 Declare the pointing function with separately named integer coordinates
  - _Requirements: 2.1_
- [ ] 3.2 Add the structured-geometry capability flag and skip the tag machinery for it
  - _Requirements: 2.2_
- [ ] 3.3 Substitute the structured prompt for a tag-based one
  ...
  - _Requirements: 2.3_
- [ ] 3.4 Pass a genuinely custom prompt through untouched
  - _Requirements: 2.6_
- [ ] 3.5 Drop a malformed geometry call and keep the spoken answer
  - _Requirements: 2.7_
- [ ] 3.6 Fix the normalised-versus-pixel assumption in the text-tag fallback
...
- [ ] 4. Split-role concurrent calls (`T1-9`)
- [ ] 4.1 Measure whether one tool-enabled call can produce both prose and a pointer
  ...
  - _Requirements: 3.1_
- [ ] 4.2 Issue the speech call with no tools declared
  - _Requirements: 3.1_
- [ ] 4.3 Issue the geometry call on a daemon thread with tools only and the minimal budget
  - _Requirements: 3.2, 3.3_
- [ ] 4.4 Skip the geometry call for a conceptual question; always attempt it in annotation mode
  - _Requirements: 3.4, 3.5_
- [ ] 4.5 Make the harvest idempotent across both accessors in either order
  - _Requirements: 3.6_
- [ ] 4.6 Contain every geometry failure; never propagate into speech
  - _Requirements: 3.7_
- [ ] 4.7 Withhold the knowledge base from the geometry call
  - _Requirements: 3.8_

- [ ] 5. Thinking budgets (`T1-7`)
- [ ] 5.1 Write the query classifier as a pure function, testing diagnostic intent first
  - _Requirements: 4.1, 4.2_
- [ ] 5.2 Map the three classes to budgets and clamp per model family
...
- [ ] 5.3 Force the minimal budget on the geometry call
  - _Requirements: 4.5_
- [ ] 5.4 Raise the geometry budget and append inspection guidance under agentic vision
  - _Requirements: 4.6_
- [ ] 5.5 Add a test asserting the two copies of the directional word list stay in sync
  - _Requirements: 4.7_

- [ ] 6. Knowledge-base context caching (`T1-6a`)
- [ ] 6.1 Verify live that a max-size payload caches before building anything
  - 60,000 characters measured at 10,002 tokens; caching served 10,008 of 10,013 prompt tokens
  - _Requirements: 5.1_
- [ ] 6.2 Key the cache on application name plus a content hash
  - _Requirements: 5.2_
- [ ] 6.3 Suppress inline injection when a cache resolved, and omit the system instruction
  - _Requirements: 5.3, 5.4_
- [ ] 6.4 Degrade every failure path to inline injection
  - _Requirements: 5.5_
- [ ] 6.5 Apply the cache to the speech call only, and record why one cache cannot serve both
  - _Requirements: 5.6_
- [ ] 6.6 Delete every live cache on client shutdown
  - _Requirements: 5.7_
- [ ] 6.7 Estimate tokens locally, pessimistically, with the measured ratio recorded
  - _Requirements: 5.8_

- [ ] 7. Grid fallback
- [ ] 7.1 Implement the coarse first pass with numbered cells and a JSON-only reply contract
  - _Requirements: 6.1, 6.2_
- [ ] 7.2 Implement the fine second pass over the chosen cell plus one cell of context
  - _Requirements: 6.3_
- [ ] 7.3 Fall back to the first pass's cell centre when the second pass fails
  - _Requirements: 6.4_
- [ ] 7.4 Honour the conceptual sentinel by placing no pointer
  - _Requirements: 6.5_
- [ ] 7.5 Upscale a small crop so grid labels stay legible to a weak model
  - _Requirements: 6.6_
- [ ] 7.6 Distinguish decode failure, transport failure and an unparseable reply in diagnostics
  - Previously every one of the three returned the same `None` and logged the same line
  - _Requirements: 6.7_
- [ ] 7.7 Skip the locator entirely on a cancelled turn
  - _Requirements: 6.8_
- [ ] 7.8 Add the native-resolution refinement crop with a keep-the-original failure path
  - _Requirements: 6.4_

- [ ] 8. Per-application prompt addenda (`T2-5`)
- [ ] 8.1 Build the addendum table keyed on sanitised executable basenames
...
- [ ] 8.2 Provide one entry point that appends, making the rule structural
  - _Requirements: 7.2, 7.6_
- [ ] 8.3 Return an empty string for an unknown or failed application name
  - _Requirements: 7.4, 7.5_
- [ ] 8.4 Switch prompt matching from equality to prefix, and unify the geometry decision with it
...
- [ ] 9. Vertex AI backend
- [ ] 9.1 Add the project and region settings, read fresh rather than cached at import
  - _Requirements: 8.4_
- [ ] 9.2 Construct the client against Vertex when a project is set, passing no key
  - _Requirements: 8.1, 8.2_
- [ ] 9.3 Check the project before the key shape in the factory
  - _Requirements: 8.3_
- [ ] 9.4 Coerce a blank region to the global endpoint
  - _Requirements: 8.5_
- [ ] 9.5 Apply the identical switch on the speech-to-speech path
  - _Requirements: 8.6_
- [ ] 9.6 Expose which backend is live to the Settings and Account pages
  - _Requirements: 8.7_
- [ ] 9.7 Widen the injected factory call only when Vertex is configured
...
- [ ] 10. Experimental capabilities, honestly labelled
- [ ] 10.1 Default every capability off, with the one argued exception
  - _Requirements: 9.1_
- [ ] 10.2 Make each reachable from Settings rather than leaving it unreachable
  - _Requirements: 9.2_
- [ ] 10.3 Write each description with its trade-off stated
...
- [ ] 10.4 Persist an explicit on or off rather than deleting the key
  - _Requirements: 9.5_
- [ ] 10.5 Collect citations for the diagnostic log and keep them out of spoken text
  - _Requirements: 9.6_
- [ ] 10.6 Register both lazily-imported satellite modules in the spec and the selftest
  - Both shipped invisible to PyInstaller's static graph before this. This is the gap
  - _Requirements: 10.2_

- [ ] 11. Grounding measurement (`T1-8`)
- [ ] 11.1 Build the fixture labeller and the accuracy harness
...
- [ ] 12.2 `--selftest` prints `SELFTEST OK`, including both lazy satellite modules
- [ ] 12.3 Fully-local regression gate: Ollama plus local speech in and out, no keys, no network
- [ ] 12.4 Live end-to-end run on the native path
- [ ] 12.5 Write the tests for this feature - 333 declared functions
