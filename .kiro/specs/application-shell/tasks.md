...
- [x] 1. The theme module (§2)
- [x] 1.1 Put every colour, spacing step, radius, duration and easing curve in one module
  - Values, not vibes: the shell, the panel and the overlay cannot drift apart if there is one source
  - _Requirements: 16.1, 16.14_
- [x] 1.2 Build one stylesheet applied once on the application, rather than per-widget styling
  - Covers menus and dialogs too, which per-widget styling would miss
  - _Requirements: 16.14_
- [x] 1.3 Add `contrast_ratio` as a pure function and assert every text-on-surface pair
  - Caught the muted text colour at 3.49:1 against a 4.5:1 requirement
  - _Requirements: 16.14_
- [x] 1.4 Add the reduced-motion resolution: follow the system by default, allow an override
...
- [x] 1.5 Add `focus_visible_only` so a mouse click leaves no dotted outline
...
- [x] 1.6 Add the no-literal-colour guard over the generated stylesheet
  - _Requirements: 16.14_

- [x] 2. The frameless frame (`S-1`)
- [x] 2.1 Create the package rather than one module, with a lazy attribute hook for the window class
  - A single module covering five pages plus a custom title bar becomes unreviewable fast
  - _Requirements: 1.7, 1.8_
- [x] 2.2 Register every package module in **both** the bundler's hidden imports and the selftest list
  - The lazy hook is invisible to the static graph, which is exactly the gap that caught two modules
    in an earlier tier
  - _Requirements: 1.8_
- [x] 2.3 Build the title bar emitting intent rather than acting on the window
  - A title bar that reached past the window to close it would bypass hide-to-tray
  - _Requirements: 5.4_
- [x] 2.4 Hand the drag to the operating system's move loop
...
- [x] 2.5 Paint the window-button glyphs instead of typing them
...
- [x] 2.6 Swap to a restore glyph when maximised, so the button says what it will do next
  - _Requirements: 16.10_
- [x] 2.7 Make the window buttons inset chips with a border and a resting background
...
- [x] 2.8 Nudge the wordmark down by the font descent and tighten the mark-to-wordmark gap
  ...
  - _Requirements: 16.11_
- [x] 2.9 Add the accent divider as its own one-pixel widget rather than a border
  ...
  - _Requirements: 16.14_
- [x] 2.10 Empty the title bar's own stylesheet function and move its rules into the theme
  ...
  - _Requirements: 16.14_
- [x] 2.11 Implement close-to-tray with a one-time notification signal
  - _Requirements: 5.1, 5.2, 5.5_

- [x] 3. Navigation rail (`S-1`)
- [x] 3.1 Define the navigation entries as one ordered list, consumed by both the rail and the stack
  - Makes an item without a page impossible by construction rather than by vigilance
  - _Requirements: 7.1, 7.2_
- [x] 3.2 Give each item its own page name, never an index
  - Index lookups are what break when someone reorders the list
  - _Requirements: 7.4_
- [x] 3.3 Emit a page request on user click only; keep the programmatic path silent
  - So a page change cannot echo back into another page change
  - _Requirements: 7.5_
- [x] 3.4 Add the configurable side, defaulting to left, with the divider facing the content
...
- [x] 3.5 Make the selection marker a child of the rail so it can travel between items
...
- [x] 3.6 Add the status footer with the privacy chip and the chat switch
  - _Requirements: 8.5, 8.6_
- [x] 3.7 Fix the chip's label and vary only its indicator colour
  ...
  - _Requirements: 8.7_
- [x] 3.8 Use the danger colour when the guard is off, not a warning colour
...
- [x] 3.9 Reduce the footer to one chip and remove the rail's wordmark
...
- [x] 3.10 Add `test_every_nav_item_maps_to_a_page`
  - _Requirements: 7.3_

- [x] 4. Resize, snap, and the two reversals (`S-1`)
- [x] 4.1 Restore the sizing, maximise and minimise style bits after the first show
...
- [x] 4.2 Apply the change with a frame-changed flag and read the style word back
...
- [x] 4.3 Declare explicit argument and return types for every call
  ...
  - _Requirements: 2.10_
- [x] 4.4 Return a boolean and never raise, so the fallback is "no snap"
  - _Requirements: 2.12_
- [x] 4.5 Verify that suppressing the returning frame is unnecessary, and pin the measurement
...
- [-] 4.6 Override the native event handler to hit-test the frame
  ...
  - _Requirements: 2.13_
- [x] 4.7 Replace the window-level cursor hit-test with eight per-edge grips
...
- [x] 4.8 Give the corners a target larger than the border
  - The same fix the chat panel needed, where two thin crossing strips left a corner nobody could hit
  - _Requirements: 3.5_
- [x] 4.9 Make the grips transparent to painting but not to the mouse, and hide them when maximised
  - _Requirements: 3.6, 3.7_
- [x] 4.10 Delete the dead hit-test helper and replace the test that still referenced it
  - It survived the refactor because one test still named it, which is how dead code persists
  - _Requirements: 3.8_
- [x] 4.11 Keep the native corner grip as the visible affordance and the no-native-handle fallback
  - Raised last so it stays above the bottom-right grip
  - _Requirements: 2.14_

- [x] 5. Pages (`S-2`)
- [x] 5.1 Build Home: power card dominant, provider, hotkey, recent table, privacy card
...
- [x] 5.2 Build Knowledge, Journal and Account against injected sources
  - _Requirements: 1.3_
- [x] 5.3 Build the Settings page as a host for the extracted widget, with its own save action
  - _Requirements: 11.4_
- [x] 5.4 Render an em dash rather than zero wherever a provider is absent or raises
...
- [x] 5.5 Never poll: refresh on page change and on an explicit refresh call only
  - _Requirements: 9.3_
- [x] 5.6 Document each number's integration requirement, including what existing data is insufficient
  - So a blank number is honestly blank rather than mysteriously blank
  - _Requirements: 9.5_
- [x] 5.7 Record the pages deliberately not built, with reasons
  ...
  - _Requirements: 8.12_
- [x] 5.8 Ignore unknown page names and swallow a page's refresh exception
  - Both are reachable from signals, and a typo in a tray action must not take the window down
  - _Requirements: 7.6, 7.7_

- [x] 6. The push-to-talk chord guard (`T2-7` follow-up)
- [x] 6.1 Install a do-nothing shortcut on the configured chord
...
- [x] 6.2 Use a shortcut rather than an application-wide event filter
...
- [x] 6.3 Build the guard from the configured chord, falling back to the default on a parse failure
  - So a user who remapped push-to-talk is protected by the same guard
  - _Requirements: 6.7, 6.8_
- [x] 6.4 Scope the guard to this window, and record why the chat panel needs none
  - It accepts no focus anywhere, so it never receives a key event at all
  - _Requirements: 6.9_
- [x] 6.5 Add the lift so Settings can record the chord the user is already bound to
  - Without it the capture button appears to ignore that chord, because the guard consumes it first
  - _Requirements: 6.10_
- [x] 6.6 Make the window itself take the initial focus
...
- [x] 6.7 Test the guard against every focusable widget, not just the one where it was found
  - _Requirements: 6.1_

- [x] 7. Window assembly
- [x] 7.1 Make every data source an injected callable and every action an outbound signal
...
- [x] 7.2 Compose the title bar, the accent rule, the rail and the page stack
  - _Requirements: 7.2_
- [x] 7.3 Scope the texture overlay to the content area and make it mouse-transparent
...
- [x] 7.5 Give the page stack an explicit opaque fill rather than transparency
  - What stops a half-painted child leaving remnants on a page change — the same class of artefact
    that retired the fade
  - _Requirements: 16.7_
- [x] 7.6 Default the window to opening at startup, and open it on an unreadable configuration too
...
- [x] 8. Settings extraction (`S-4`)
- [x] 8.1 Move the builder's body into a plain widget with no host of its own
...
- [x] 8.2 Keep the widget free of any scroll area and any button box
...
- [x] 8.3 Expose validity, save and local-data-cleared as signals, and save as a boolean call
...
- [x] 8.4 Host it from both the first-launch modal and the shell page
  - One implementation, two hosts
  - _Requirements: 11.4_
- [x] 8.5 Make the shell **react** to local data being cleared, not merely record it
  - _Requirements: 11.8_
- [x] 8.6 Prove the extraction by running every pre-existing settings test unmodified
  - The acceptance criterion. If any needed editing, it was a rewrite
  - _Requirements: 11.2_
- [x] 8.7 Scope local-data clearing and return failures rather than raising
...
- [x] 9. Restart labelling (`T4-7`)
- [x] 9.1 Declare the explicit set of restart-requiring settings, excluding API keys
...
- [x] 9.2 Append a marker glyph to each such label, and build the explanatory note from the constant
...
- [x] 9.3 Make the marker lookup a pure function and assert coverage in both directions
  - So labelling is testable without constructing the dialog, and a setting cannot be marked
    inconsistently in two places
  - _Requirements: 13.5, 13.6_
- [x] 9.4 Choose the glyph by measuring ink height against surrounding cap height at three sizes
...
- [x] 9.5 Verify the glyph is real and does not clip
...
- [x] 10. Power control wiring (`S-3`)
- [x] 10.1 Keep the listening state in one place and make the application its only writer
  - The window emits a request; it never writes the flag itself
  - _Requirements: 10.1, 10.2_
- [x] 10.2 Drive all three views from one change signal, with no view holding its own copy
  - _Requirements: 10.3, 10.4_
- [x] 10.3 Make the window's accessor read through to the provider rather than caching
  - With a provider wired up the source wins, so a caller cannot make a view show something the
    source disagrees with
  - _Requirements: 10.5, 10.6_
- [x] 10.4 Verify the toggle is instant and needs no restart
  ...
  - _Requirements: 10.7_
- [x] 10.5 Stop speech in progress when pausing, mirroring the existing pause behaviour
  - _Requirements: 10.8_
- [x] 10.6 Apply the same arrangement to the chat panel's visibility switch, and re-read after asking
...
- [x] 11. Tray relationship (`S-5`)
- [x] 11.1 Keep the tray, with show, pause and quit
...
- [x] 11.2 Show and raise the window on a left click
  - _Requirements: 15.2_
- [x] 11.3 Remove the items that now have a better home in the window
  - A menu that duplicates the window is two places to keep in sync and two places to fix a bug
  - _Requirements: 15.4_
- [x] 11.4 Make the tray's pause item read the same source, with no second boolean anywhere
  - _Requirements: 15.6_
- [x] 11.5 Raise the inherited actions as window signals, since only the application can service them
  - _Requirements: 15.7_
- [x] 11.6 Route the tray's quit and the account page's quit into one shutdown sequence
  - _Requirements: 5.3_

- [x] 12. Sizing and the scroll hosts
- [x] 12.1 Clamp both the opening size and the minimum against the available screen geometry
...
- [x] 12.2 Recompute the minimum when the window moves
  - Otherwise a floor measured on a large panel follows the window onto a small one
  - _Requirements: 4.4_
- [x] 12.3 Put each page except Settings in its own scroll area, and lower the floor
...
- [x] 12.4 Leave Settings unwrapped, and assert it has exactly one scroll area
  - Wrapping it again would nest one scrolling region inside another and put the save action back
    below the fold
  - _Requirements: 4.8, 12.9_
- [x] 12.5 Take the scroll areas out of the tab order
  ...
  - _Requirements: 4.9_
- [x] 12.6 Size the settings dialog from the **page's** natural height, clamped to the screen
...
- [x] 12.7 Add the parametrised small-screen fit check and the control inventory test
...
- [x] 13.2 Every pre-existing settings test passing **unmodified**
- [x] 13.3 `--selftest` prints `SELFTEST OK` with every package module in the runtime list
- [x] 13.4 Contrast measured for every text-on-surface pair against the 4.5:1 requirement
- [x] 13.5 Manual smoke test: drag, snap to each edge, resize from all eight regions, maximise, restore
- [x] 13.6 Manual smoke test: drag between two monitors at different scaling, confirm nothing jumps
- [x] 13.7 Manual smoke test: open the window, immediately press the chord — Nimbus listens, not pauses
- [x] 13.8 Manual smoke test: close the window, confirm push-to-talk still works and the balloon showed once
- [x] 13.9 Manual smoke test: open Settings on a 1366×768 display and confirm Save is reachable
- [x] 13.10 Write the tests for this feature - 319 declared functions
...
- [x] 14. The settings resolution chain
- [x] 14.1 Resolve every setting through one function, in the order environment, store, default
  - _Requirements: 17.1_
- [x] 14.2 Write a value through to the credential store when it came from the environment
  - A one-shot migration out of a dotfile, so a value set once survives without that file
  - _Requirements: 17.2_
- [x] 14.3 Record the consequence at the call site rather than leaving it to be discovered
...
- [x] 14.4 Give API keys a separate uncached path, read per request
  - Which is why they are deliberately absent from the restart set: a new key works immediately
  - _Requirements: 17.5_
- [x] 14.5 Make resolution total: an unreadable store falls through to the declared default
