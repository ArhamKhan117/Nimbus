...
- [ ] 1. Capture exclusion, measured before anything is built (`S-7`)
- [ ] 1.1 Implement the exclusion call, returning a boolean and never raising
  - A ctypes failure must degrade to the hide-and-show cycle, not crash on every show
  - _Requirements: 1.1, 1.8_
- [ ] 1.2 Measure the affinity against an opaque window and a translucent one
  ...
  - _Requirements: 2.2_
- [ ] 1.3 Make the body **opaque** and record the trade
...
- [ ] 1.4 Verify effectiveness by pixel count, **with a control**
...
- [ ] 1.5 Re-apply exclusion on every show event rather than once at construction
  - A window can lose it
  - _Requirements: 1.2_
- [ ] 1.6 Name the region-blanking mode in the source and record why it was rejected
  ...
  - _Requirements: 1.5_
- [ ] 1.7 Keep rounded corners via a rounded window region, re-applied on every resize
...
- [ ] 1.8 Expose the pre-support query plus no-op hide and show methods
  - So the existing overlay hide cycle can cover the panel too, and the calls can be unconditional
  - _Requirements: 1.6, 1.7_
- [ ] 1.9 Prohibit window-level opacity animation and record the reason
...
- [ ] 2. Pure geometry and state (`S-6`)
- [ ] 2.1 Implement size clamping into a bounded range
...
- [ ] 2.2 Implement top-centre positioning in the target screen's logical coordinates
  - _Requirements: 5.10_
- [ ] 2.3 Raise the default size and grow the interior padding with it
  ...
  - _Requirements: 5.1_
- [ ] 2.4 Set the resize border to double as the visible bezel, matching the shell window's width
...
- [ ] 2.5 Give the corners a hit region several times the border
...
- [ ] 2.6 Raise the header, footer and state-strip heights
  - The strip in particular read as a rendering artefact rather than the deliberate indicator it is
  - _Requirements: 5.11_
- [ ] 2.7 Map each interaction state to a strip colour, falling back to idle for anything unknown
  ...
  - _Requirements: 5.11_
- [ ] 2.8 Remember the chosen size per monitor
  - _Requirements: 5.5_

- [ ] 3. Pure session functions (`S-8`)
- [ ] 3.1 Derive a session title from the first user message with **no model call**
...
- [ ] 3.2 Require **both** an application change and an idle period for an automatic new session
...
- [ ] 3.3 Rebuild history in exactly the shape the pipeline worker appends
  - Anything else would work until the first provider that actually reads history, which is the worst
    time to find out
  - _Requirements: 12.1_
- [ ] 3.4 Drop system messages from the rebuild
  ...
  - _Requirements: 8.4_
- [ ] 3.5 Apply the image budget newest-first, **before** any blocks are built
...
- [ ] 3.6 Duplicate the exchange-window constant rather than importing it, and pin the pair
  - Importing the orchestrator here would drag a whole running application into a module that must be
    testable on its own
  - _Requirements: 12.2, 12.3, 12.4_
- [ ] 3.7 Read the image budget at call time rather than import time
  - So a settings change applies without a restart
  - _Requirements: 12.5_

- [ ] 4. The window (`S-6`)
- [ ] 4.1 Build one frameless, always-on-top, never-focusable window
  - _Requirements: 4.1, 4.2_
- [ ] 4.2 Accept content only through inbound signals from the three producing threads
  - All visual work on the main thread, the same way the caption reaches the overlay
  - _Requirements: 4.7_
- [ ] 4.3 Wrap every public entry point so an exception degrades to "no chat panel"
...
- [ ] 4.4 Record why the panel needs no keyboard-chord guard
  - It accepts no focus anywhere, so it never receives a key event at all
  - _Requirements: 4.3_
- [ ] 4.5 Build the state strip, header, scrolling transcript and footer
  - _Requirements: 5.11_
- [ ] 4.6 Style the hairlines from the shared theme rule the shell's divider uses
  - So the two surfaces cannot end up with different dividers
  - _Requirements: 5.11_
- [ ] 4.7 Explain the core interaction in the empty state, using the **real** configured chord
...
- [ ] 5. Session store (`S-8b`)
- [ ] 5.1 Add three tables to the **existing** database, purely additively
...
- [ ] 5.2 Mirror the existing store's structure: autocommit, row-by-name, connection per method
  - Nothing held across turns, so a crash cannot leave a write transaction open
  - _Requirements: 9.4_
- [ ] 5.3 Document the single-writer rule and make it structural rather than a comment
  - The panel — not the pipeline worker — calls the message write, and the panel lives on the main thread
    by definition
  - _Requirements: 9.5, 9.6_
- [ ] 5.4 Omit the foreign key deliberately, and make the cascade explicit
...
- [ ] 5.5 Derive the screenshot root from the database path
  ...
  - _Requirements: 9.9_
- [ ] 5.6 Define the three roles, and justify the system role as load-bearing
...
- [ ] 5.7 Extend an open reply by an update per delta rather than one write at the end
...
- [ ] 5.8 Re-read a message immediately after writing it
  ...
  - _Requirements: 9.12_
- [ ] 5.9 Implement the three screenshot refusals, in priority order
...
- [ ] 5.10 Exclude the image and the suppression flag from persistence and comparison
  - They carry the pixels only as far as the main-thread write call
  - _Requirements: 10.8_
- [ ] 5.11 Draw the coordinate marker at the same radius and stroke as the diagnostic screenshot's
...
- [ ] 5.12 Delete screenshots with their session, and prune by retention at startup
...
- [ ] 5.13 Treat a coordinate as absent unless **both** columns are present
  - A half-null pair read as a coordinate with a zero component would fly the cursor somewhere the model
    never suggested
  - _Requirements: 12.8_
- [ ] 5.14 List sessions most-recently-used first, with a search over title and application name
...
- [ ] 5.15 Set the title at the first user message
  - The earliest moment it is knowable, and it costs nothing
  - _Requirements: 13.7_
- [ ] 5.16 Add the backward-compatibility gate asserting the existing tables are untouched
  - _Requirements: 9.3_
- [ ] 5.17 Make new-session and switch-session mutate the caller's history **in place**
...
- [ ] 5.18 Record that the store is a record, not the source of truth
...
- [ ] 6. Collapse (`S-6b`)
- [ ] 6.1 Add the collapsed state, keeping the panel's width and position
...
- [ ] 6.2 Derive the collapsed height from the live layouts
...
- [ ] 6.3 Set the height explicitly rather than relying on hiding children
  - A frameless window keeps its old height if nothing tells it otherwise, leaving an empty rectangle
  - _Requirements: 6.5_
- [ ] 6.4 Keep the bar exactly where the panel's top edge was
...
- [ ] 6.5 Decide the expansion direction before anything moves, and remember it
  - Deciding again on expand would let a panel dragged near a screen edge mid-collapse expand the other
    way and jump
  - _Requirements: 6.8, 6.9_
- [ ] 6.6 Choose the direction from the panel's position, defaulting downwards with no screen
...
- [ ] 6.7 Record the pre-collapse height **before** hiding the children
  ...
  - _Requirements: 6.12_
- [ ] 6.8 Block signals while syncing the collapse control
  ...
  - _Requirements: 6.13_
- [ ] 6.9 Make the glyph an arrow pointing where the body will go
  - Not a state indicator
  - _Requirements: 6.14_

- [ ] 7. Transcript and session rows (`S-8`)
- [ ] 7.1 Build the session row as a button with a two-line label
  ...
  - _Requirements: 7.2_
- [ ] 7.2 Build the session row as a plain frame containing its own labels
...
- [ ] 7.3 Render message rows with the thumbnail and its coordinate marker
  - _Requirements: 10.9_

- [ ] 8. Interactions (`S-6b`)
- [ ] 8.1 Add replay, re-point, retry and new chat
  ...
  - _Requirements: 11.4_
- [ ] 8.2 Add the session picker with search and delete
  - _Requirements: 13.8_
- [ ] 8.3 Add the "that was wrong" flag, and make it do something
...
- [ ] 8.4 Add pinning so the idle timer does not dismiss the panel
  - _Requirements: 16.7_
- [ ] 8.5 Add the idle auto-hide with a bounded setting, zero meaning never
  - _Requirements: 16.4_

- [ ] 9. Durable counters
- [ ] 9.1 Record privacy suppressions from the single capture choke point, never raising
...
- [ ] 9.2 Derive question counts from the stored messages rather than a separate tally
  - So the home page's count and the transcript cannot disagree about what happened
  - _Requirements: 14.3_
- [ ] 9.3 Read recent activity from the store rather than an in-memory list
  ...
  - _Requirements: 14.4_
- [ ] 9.4 Count and list only user messages, and do not join the reply
...
- [ ] 9.5 Return a real date-time where the stored value parses, the string where it does not
  - So relative formatting is possible without guessing at an unparseable value
  - _Requirements: 14.7_
- [ ] 9.6 Return zero or an empty list on any query failure
  - A status table is not worth taking the window down for
  - _Requirements: 14.8_

- [ ] 10. Show, hide, and the no-fade rule
...
- [ ] 10.2 Slide the entrance instead, animating position
  - Needs no offscreen buffer
  - _Requirements: 3.2_
- [ ] 10.3 Clear any graphics effect on reveal rather than trusting none is attached
  - That is precisely the state that caused the bug
  - _Requirements: 3.6_
- [ ] 10.4 Make dismissal immediate
  - There is no fade left to run
  - _Requirements: 3.7_
- [ ] 10.5 Stop and dispose of the previous animation before starting its replacement
  - Two live animations on the same property fight, and the loser wins intermittently
  - _Requirements: 3.9_

- [ ] 11. Integration
- [ ] 11.1 Wire the application's three outbound signals into the panel's inbound ones
  - _Requirements: 4.7_
- [ ] 11.2 Route the toggle and new-chat shortcuts through the existing global listener
  - Not a second low-level hook, and not through the chord parser, which deliberately rejects chords it
    cannot own
  - _Requirements: 16.5_
- [ ] 11.3 Make the application the single writer of the panel's visibility
  ...
  - _Requirements: 16.6_
- [ ] 11.4 Suppress the caption overlay while the panel is showing the transcript
...
- [ ] 11.5 Pass the capture through the message and let the store decide
  - The suppression flag must be the **same** boolean the guard returned, or the protection is decorative
  - _Requirements: 10.3_
- [ ] 11.6 Make the overlay hide-and-show slots call the panel's, unconditionally
  - Both are no-ops when exclusion is active
  - _Requirements: 1.6, 1.7_
- [ ] 11.7 Register both modules in the bundler's hidden imports and the selftest's runtime list
  - _Requirements: 4.1_
- [ ] 11.8 Prune sessions at startup, best effort
  - _Requirements: 10.12_

- [ ] 12. Tests and verification
- [ ] 12.1 Full suite green with the dotenv neutralisation, zero regressions
- [ ] 12.2 `--selftest` prints `SELFTEST OK` with both modules in the runtime list
- [ ] 12.3 Capture exclusion verified by pixel count **with its control**, five runs
- [ ] 12.4 Manual: ask with the panel visible — the answer is about the application, not the panel
- [ ] 12.5 Manual: reopen after dismissing — the background is not black
- [ ] 12.6 Manual: switch session — no black bars down the sides
- [ ] 12.7 Manual: collapse near the bottom — the body opens upwards and the bar does not move
- [ ] 12.8 Manual: resize from each corner — the diagonal cursor appears at every one
- [ ] 12.9 Manual: screenshots on, ask in front of a password manager — no image lands on disk
- [ ] 12.10 Write the tests for this feature - 236 declared functions
