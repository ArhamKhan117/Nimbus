"""Nimbus speech-to-text layer.

AssemblyAIStreamingSTT -- cloud streaming via the AssemblyAI
...
P50 finalization latency, which is the dominant term in the end-to-end
latency budget.

Providers are added by subclassing STT (do not rewrite the protocol):
- ``FasterWhisperSTT``: local offline CT2 Whisper for privacy / offline users.
...
- ``app.py`` calls ``start()`` on hotkey press and ``stop()`` on
...
Top-to-bottom order (so ``python -m stt`` works):
    1. Module docstring
    2. Imports
    3. Constants
...
class STT(ABC):
    ...
    Concrete implementations: :class:`AssemblyAIStreamingSTT` (cloud
    streaming) and :class:`FasterWhisperSTT` (local offline). Do not break
    this shape -- a provider swap is supposed to be a subclass, not a refactor.
    """
    ...
    def on_partial_transcript(self, callback: Callable[[str], None]) -> None:
        ...
        Used for optional debug printing; a live caption overlay could also
        consume it. Thread safety: the callback runs on the
...
class AssemblyAIStreamingSTT(STT):
    """STT using AssemblyAI Universal-3 realtime-pro streaming.
    ...
    natural VAD end-of-turn detector. This is the dominant latency term.
    ...
    - ``start()`` / ``stop()`` are called on the Qt main thread (or any
      thread -- ``app.py`` may call from a worker).
    ...
    def __init__(
        ...
        Args:
            ...
            speech_model: ``u3-rt-pro`` by default (low-latency realtime).
            sample_rate: 16000 Hz PCM16 mono (AssemblyAI u3-rt-pro requirement).
            chunk_frames: 1024 frames per audio callback block.
        ...
        # TTS-to-mic feedback-loop suppression.
        ...
        # Audio-level (RMS) signal for the waveform widget.
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            # the debug log: "That's kind of weird." → "That's kind of—"
    ...
    def stop_recording(self) -> str:
        ...
        # authoritative signal per docs.assemblyai.com). Previous code had `else: break`
        ...
        # window is 100ms: Conservative VAD
    ...
    def on_audio_level(self, callback: Callable[[float], None]) -> None:
        ...
        pyqtSignal. Must be fast + must not raise — the
    ...
    def _on_error(self, _client, error) -> None:
        ...
        Captures the error into ``self._stream_error`` so ``stop()``
...
# --- FasterWhisperSTT (local offline provider) ----------------------
...
if __name__ == "__main__":
    # Manual live-API verification. Run: py -3.13 -m stt
    ...
    print("=" * 70)
    print("Nimbus -- stt.py manual verification")
    print("=" * 70)
    print(
