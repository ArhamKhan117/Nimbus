"""Nimbus persistent memory — plain markdown + SQLite index.

This module gives Nimbus a human-readable long-term memory per Windows app.
Most voice assistants are stateless; persistent per-app memory is what lets
Nimbus build up context about how you use each program over time.

Design philosophy: no vector DB, no embeddings, no RAG. A modern LLM is good
at reading brief per-app summaries directly at this small scale, so we keep
it simple. Just plain markdown files
(`~/.nimbus/memory/<app>.md`) that the LLM writes and reads directly,
plus a tiny SQLite index (`~/.nimbus/index.db`) for fast
...
exactly what Nimbus remembers about them — transparency is the UX contract.

Responsibility boundary:
- THIS MODULE owns the read/write API over the markdown files + SQLite index.
  It does NOT decide when to record, what to inject into Nimbus's prompt, or
  which app is currently active.
- app.py calls `recall()` before sending a Nimbus request, and
...
- Callers can use `list_known_apps()` to iterate over all known apps
  (e.g. for a periodic insights pass).
...
"human-readable, user can delete to reset" promise that makes
...
def _escape_markdown_fences(text: str) -> str:
    ...
    Nimbus's responses can contain ```python code fences; writing those
    verbatim into our markdown blocks would break the block shape when
    the file is parsed. Swap to ``'''`` — plain
...
def _escape_single_line(text: str) -> str:
    ...
    "## heading\\nfield\\nfield\\nfield" block shape that parsers
...
class MemoryStore:
    ...
    Construct once at app startup (via ``app.py``). The constructor
    ...
    **Thread safety:** each method opens its own short-lived SQLite
    ...
    handles concurrent reads cleanly; the app has ONE writer (the Qt main
    ...
    **Error handling:** this module trusts its inputs — callers pass
    ...
    def __init__(
        ...
        its own so multi-thread reads don't need lock coordination with
        the Qt main thread.

        Args:
            memory_dir: directory where per-app markdown files live.
                Defaults to ``config.MEMORY_DIR`` = ``~/.nimbus/memory``.
            index_db_path: path to the SQLite index file. Defaults to
                ``config.INDEX_DB_PATH`` = ``~/.nimbus/index.db``.
        """
    ...
    def _ensure_schema(self) -> None:
        ...
        session; no migration logic needed. Future schema
    ...
    def recall(
        ...
        decides what matters. A smarter recall could be added later if
        raw-tail proves insufficient; that's a "wait for the data" decision.
        ...
        Args:
            ...
            max_chars: hard cap on returned string length. Defaults to
                ``config.MEMORY_RECALL_MAX_CHARS`` (1500). Revisit this number
                alongside T2-4 (images in history) and T1-6a (context caching)
                as a single context-budget decision, not in isolation.
    ...
    def record(
        ...
        Called by ``app.py`` immediately after a Nimbus response
        ...
        Args:
            ...
            model_response: the natural-language text that was sent to TTS.
                Rendered in the ``Response:`` paragraph.
            pointer_targets: list of (x, y) tuples in physical pixels for
                the overlay pointer animation. Empty list means Nimbus
        ...
        escaped_response = _escape_markdown_fences(model_response)
    ...
    def list_known_apps(self) -> list[dict]:
        ...
        Each dict has keys ``{app_name, first_seen,
...
if __name__ == "__main__":
    ...
    #
    # Writes to the REAL ~/.nimbus/ dir (not a temp dir) so the user
    # can inspect the files via File Explorer and confirm the "human-readable"
    # claim. Uses a synthetic app name NIMBUS_GATE_TEST.EXE so repeated gate
    ...
    # characters that land in Nimbus responses (→, —, em-quotes, etc.).
    ...
    # app.py will need the same reconfigure at its own entry point.
    try:
        _sys.stdout.reconfigure(encoding="utf-8", errors="replace")
    except (AttributeError, OSError):
        pass  # older Python or unusual stdout — best-effort

    _TEST_APP = "NIMBUS_GATE_TEST.EXE"

    print("=" * 70)
    print("Nimbus -- memory.py manual verification")
    print("=" * 70)
    ...
    store.record(
        ...
        model_response="Click View tab, then Freeze Panes → Freeze Top Row.",
    ...
    store.record(
        ...
        model_response="Select the column, click Data tab → Sort A to Z.",
